...
const WorkingPartnerPayouts = () => {
    ...
    const viewCashHandler = (event) => {
        ...
        var database = app.database();
        database.ref().child("Franchise").child(pushid).child("Transactions")
            .once('value', function (snapshot) {
                if (snapshot.exists()) {
                    var content = [];
    ...
    const updateCashHandler = (event) => {
        ...
        let person = prompt("Please enter your cms number:", "****");
        let format = /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?abcdefghijklmnopqrstuvwxyz]+/;
if(format.test(person) ){
  alert("Please enter valid cms number ")
} else {
    if( person.length!==13 ){
        alert("Please enter valid cms number.")
    }else{
        ...
        var firebaseref1 = app.database().ref().child("Franchise").child(userid).child("Cash");
        firebaseref1.transaction(function (currentstock) {
            ...
            function (error, committed, snapshot) {
                if (error) {
                    ...
                    console.log(person)
                    console.log(userid)
                    var firebaseref = app.database().ref().child("Franchise").child(userid).child("Transactions").push();
                    ...
                    var database = app.database();
                    database.ref().child("Franchise").child(userid).child("Transactions")
                        ...
                        .once('value', function (snapshot) {
                            if (snapshot.exists()) {
                                ...
                                snapshot.forEach(function (data) {
                                    var val = data.val();
                                    app.database().ref().child("Franchise").child(userid).child("Transactions").child(val.PushId).child("Status").set("Approved");
                                })
                            }
                            Swal.fire({
...
if(format.test(person) ){
  alert("Please enter valid cms number ")
    ...
    return (
        <Fragment>
            <BreadCrumb parent={<Home />} subparent="Payouts" title="Working Partner Payouts" />
                            ...
                            <div className="col-md-11 text-right" style={{ marginTop: "-5%", marginBottom: "3%" }}>
                                <div className="dt-buttons btn-group">
                                    ...
                                    <ReactHTMLTableToExcel
                                        ...
                                        table="data-table"
                                        filename="WorkingPartnerPayouts"
                                        sheet="WorkingPartnerPayouts"
                                        buttonText="Excel" />
                                    <iframe
                                        id="iDatatable"
                                        src="/payouts/working-partner-payouts"
                                        style={{ display: 'none' }}
                                        title="Receipt"
                                    />
                                    ...
                                    <Table id="datatable">
                                        <thead>
                                            <tr>
                                                ...
                                                <th scope="col"> Working Partner ID	</th>
                                                <th scope="col"> Working Partner Name	</th>
