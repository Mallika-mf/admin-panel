...
const DriverTransactionHistory = () => {
  ...
  const onClickSearchBox = (event) => {
    ...
    app.database().ref().child("DeliveryPartner").orderByChild("UserId").equalTo(searchBox)
      .on('value', function (snapshot) {
        if (snapshot.exists()) {
          ...
          snapshot.forEach(snap => {
            userData = snap.val()
            // let TransactionName = ""
            snap.child('FTransactions').forEach(function (data1) {
  ...
  return (
    <Fragment>
      <BreadCrumb parent={<Home />} subparent="Payouts" title="Driver Transaction History" />
      <Container fluid={true}>
        <Row className="form-row" style={{ marginTop: "3%" }}>
          <Col className="form-group col-md-6">
            <label className="form-label">Enter Delivery Partner Number</label>
            <Row>
        ...
        <Row>
          <Col sm="12">
            <CardHeader>
              <h6> Delivery Partner Transaction History</h6>
        ...
        <div className="col-md-11 text-right" style={{ margin: "2%" }}>
          <div className="dt-buttons btn-group">
            ...
            <ReactHTMLTableToExcel
              ...
              table="data-table"
              filename="DriverTransectionHistory"
              sheet="DriverTransectionHistory"
              buttonText="Excel" />
            <iframe
              id="iDatatable"
              src="/payouts/driver-transaction-history"
              style={{ display: 'none' }}
              title="Receipt"
            />
              ...
              <Table id="data-table" className={classes.table}  >
                <TableHead >
                  <TableRow>
                    ...
                    <TableCell scop="col">Driver Id</TableCell>
                    <TableCell scop="col">Driver Name</TableCell>
