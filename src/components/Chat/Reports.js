...
import { ShoppingCart,
    ...
    File,
    Smile,
    Command, Cloud, Book, FileText, Server, Image, Sliders,Plus, Map, GitPullRequest, Edit, Mail, UserCheck, Layers, HelpCircle, Database, Headphones, Mic, ShoppingBag, Search, AlertOctagon, Lock, User} from 'react-feather';
...
const Reports = (props) => {
   ...
   const userReports = (event)=>{
    ...
    history.push(`${process.env.PUBLIC_URL}/reports/user-reports`);

   }
   const ChefReports = (event)=>{
    event.preventDefault();
    history.push(`${process.env.PUBLIC_URL}/reports/chef-reports`);

   }
   const DeliveryPartnerReports = (event)=>{
    event.preventDefault();
    history.push(`${process.env.PUBLIC_URL}/reports/delivery-partner-reports`);

   }

   const WorkingPartnerReports = (event)=>{
    event.preventDefault();
    history.push(`${process.env.PUBLIC_URL}/reports/working-partner-reports`);
