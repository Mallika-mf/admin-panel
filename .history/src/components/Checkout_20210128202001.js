...
class Checkout extends React.Component {
  ...
  handleAddressSelect = (address) => {
    ...
    for (let city of cities) {
      if (address.Address.includes(city.Name)) {
        ...
        this.setState({
          taxes:
            (parseInt(city.PackingCharges) + parseInt(city.DeliveryCharges)) *
            0.18,
        });
        this.setState({
          total:
  ...
  getUserCart = async () => {
    ...
    if (user.exists()) {
      ...
      if (!unmount) {
        ...
        this.setState({
          userAddresses: Object.values(
            get(user.val()[userName], "Address", [])
          ),
        });
      }

      Object.values(get(user.val()[userName], "Cart", [])).map(
        (cart) => (subTotal += parseInt(get(cart, "Total", 0)))
      );
      if (!unmount) {
  ...
  onDeliveryDateChange = (dateString) => {
    this.setState({
      deliveryDate: Moment(new Date(dateString)).format("YYYY-MM-DD"),
    });
  };
  ...
  disabledDate = (current) => {
    ...
    return (
      current.valueOf() > new Date(preOrder.OrderEndDate).valueOf() &&
      current.valueOf() < new Date(preOrder.OrderDate).valueOf()
    );
  };
  ...
  handlePromo = async () => {
    ...
    comp.setState({
      total:
        parseFloat(subTotal) +
        (parseInt(packingCharges) + parseInt(deliveryCharges)) +
        (parseInt(packingCharges) + parseInt(deliveryCharges) * 0.18),
    });
    if (!promocode) {
  ...
  handlePlaceOrder = async () => {
    ...
    if (
      currentRedius <
      this.getUserRadiusBetweenTwoLocations1(String(selectedLocation))
    ) {
    ...
    if (user.exists()) {
      ...
      if (vendor.exists()) {
        ...
        window.axios
          .post(
            "https://liveyumfoods.xyz/ordersmothers.php?amount=" +
            ...
            var options = {
              ...
              handler: function (response) {
                try {
                  ...
                  firebaseref
                    .child("Flat")
                    .set(get(userAddressSelected, "Flat", ""));
                  firebaseref
                    .child("LocationCoordinates")
                    .set(get(userAddressSelected, "Coord", ""));
                  firebaseref
                    .child("Address")
                  ...
                  firebaseref.child("Chef").set(chefId);
                  // firebaseref.child("ChefStatus").set(get(vendor.val(), 'Status', ''));
                  firebaseref
                    .child("ChefStatus")
                    .set(get(vendor.val(), "Status", ""));
                  firebaseref
                    .child("ChefName")
                    .set(get(vendor.val(), "Name", ""));
                  firebaseref
                    .child("ChefImage")
                    .set(get(vendor.val(), "PP", ""));
                  firebaseref
                    .child("ChefLoc")
                    .set(get(vendor.val(), "Location", ""));
                  firebaseref.child("ChefCity").set(get(vendor.val(), "City"));
                  firebaseref
                    .child("ChefLocality")
                    .set(get(vendor.val(), "Locality", ""));
                  firebaseref
                    .child("ChefCommision")
                    .set(get(vendor.val(), "Commision", ""));
                  ...
                  firebaseref
                    .child("RazorpayOrderID")
                    .set(response.razorpay_order_id);
                  firebaseref
                    .child("RazorpaySignature")
  ...
  handlePaytm = (event) => {
    ...
    window.axios
      .post(
        "https://liveyumfoods.xyz/Paytm/test1.php?code=https://securegw.paytm.in/theia/paytmCallback?ORDER_ID=" +
          orderid +
          "&MID=MOTHER88276112838117&ORDER_ID=" +
          orderid +
          "&AMOUNT=" +
          parseFloat(100)
      )
            ...
            <Row>
              ...
              <Col md={4}>
                <div className="generator-bg rounded shadow-sm mb-4 p-4 osahan-cart-item">
                  ...
                  <div className="mb-2 bg-white rounded p-2 clearfix">{console.log(typeof(`im here${Date.now()},${localStorage.getItem("chefId")},${parseFloat(total).toFixed(2)},${parseFloat(taxes).toFixed(2)}`))}
