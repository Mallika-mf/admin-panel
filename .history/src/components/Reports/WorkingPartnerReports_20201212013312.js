...
const onUpdateChef=(event)=>{
  console.log(event.target.id)
  let chefid = event.target.id
  app.database().ref().child("CloudKitchen")
  .orderByChild("Franchise").equalTo(chefid)
  .once("value", function(snapshot) {
    if(snapshot.exists()){
        app.database().ref().child("Franchise").child(chefid).child("Chefs").set(parseInt(snapshot.numChildren()));

    }else{
              app.database().ref().child("Franchise").child(chefid).child("Chefs").set(parseInt(0));

    }
             console.log("Chefs",snapshot.val())
  });
}
...
let excludeSearch = ["AlternateNumber","Created", "Status", "Address","City","State","Zipcode","AccountName","AccountNumber","BranchName","BranchAddress","IFSC","Cash","Commision","Updated","AStatus","City1","Doc1","Doc2","Doc3","Doc4","Chefs"]
