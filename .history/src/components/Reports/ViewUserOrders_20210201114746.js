...
const RefundOrderTable = () => {
    ...
    useEffect(() => {
        ...
        database.ref().child("Orders").orderByChild("UserId").equalTo(userid)
        ...
        var total = 0;
        app.database().ref().child("Orders").orderByChild("UserId").equalTo(userid).once("value", function (snapshot) {
            if (snapshot.exists()) {
            ...
            if (snapshot.exists()) {
                snapshot.forEach(function (data) {
                    ...
                    if (val.Discount != null)
                        discounts = discounts + +parseFloat(val.Discount);
                });
                setDiscount("₹" + parseFloat(discounts).toFixed(2));
            }
        });
        ...
        app.database().ref().child("Orders").orderByChild("UserId").equalTo(userid).once("value", function (snapshot) {
            if (snapshot.exists()) {
                order = snapshot.numChildren();
                setOrders(parseInt(order));
            }
        });
            ...
            <Container fluid={true}>
                <Row>
                    ...
                    <Input type="text"   placeholder="Search for Name, Number, OrderId..." onChange={handleSearch}  required=""  className="form-control"  /> */}
                            <h6><b>Total Orders:&nbsp;</b>{orders}</h6>
                            <h6><b>Total Amount:&nbsp;</b>{total}</h6>
                            <h6><b>Toal Discount Amount</b>&nbsp;{discount}</h6>
                    ...
                    <div className="col-md-6 text-right" style={{ margin: "3%" }}>
                        <div className="dt-buttons btn-group">
                            ...
                            <ReactHTMLTableToExcel
                                ...
                                table="datatable"
                                filename="CustomerOrders"
                                sheet="CustomerOrders"
                                buttonText="Excel" />
                            <iframe
                                id="iDatatable"
                                src="/reports/customer-order-report"
                                style={{ display: 'none' }}
                                title="Receipt"
                            />
                                        ...
                                        {recordsAfterPagingAndSorting().map((item, id) => {
                                            ...
                                            return (
                                                <tr key={id}>
                                                    ...
                                                    {item.Status === "1" ?
                                                        <td style={{ color: "orange" }} >{"New Oder"}</td> : item.Status === "2" ?
                                                            <td style={{ color: "	#FFFF00" }}>{"Processing Order"}</td> : item.Status === "3" ?
                                                                <td style={{ color: "red" }}>{"Waiting For Pickup"}</td> : item.Status === "4" ?
                                                                    <td style={{ color: "gray" }}>{"Pickedup"}</td> : item.Status === "5" ?
                                                                        <td style={{ color: "green" }}>{"Delivered Order"}</td> : item.Status === "10" ?
