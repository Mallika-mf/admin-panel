...
const ChefProfile = () => {
    ...
    const [itemDetail, setDetails] = useState('')
    ...
    const onSaveDetailsHandler = (event) => {
        let pushId = event.target.id

        app.database().ref().child("CloudKitchen").child(sname).child("FoodItems").child(pushId).child("Details").set(itemDetail)
        Swal.fire({
    ...
    const onSaveNameHandler = (event) => {
        let pushId = event.target.id

        app.database().ref().child("CloudKitchen").child(sname).child("FoodItems").child(pushId).child("Name").set(itemName)
        Swal.fire({
            title: "Successfully Save!",
            icon: "success",
            confirmButtonText: "Ok"
    ...
    const onChangeFoodItemName = (event) =>{
        if(sname===""){
            alert('Please Enter Chef Id')
            return;
        }
                    ...
                    <div className="col-md-12">
                        ...
                        <div className="card mb-4">
                            ...
                            <div className="card-body">
                                ...
                                <div className="form-group col-md-3">
                                        <input type="text" value={searchItem} onChange={onChangeFoodItemName} className="form-control"  placeholder="Search  Fooditem.."/>
                                    </div>
                                <div id="tablepdf">
                                    <div className="table-responsive text-nowrap">
                                        <table className="datatables-demo table table-striped table-bordered" id="data-table">
                                            ...
                                            <tbody id="datatable-food">
                                                {foodItems.filter(order=>order.Name.toString().toLowerCase().trim().includes(searchItem)).map((items, id) => {
                                                    return (
                                                        <tr key={id}>
                                                            ...
                                                            <td><input type="text" id={items.PushId} value={items.Name} onChange={onChangeNameHandler} className="price" /></td>
                                                            <td><Button type="button" id={items.PushId} onClick={onSaveNameHandler} className="warning"><Check id={items.PushId} onClick={onSaveNameHandler} size={20} /></Button></td>                                                            <td>{items.Category}</td>
                                                            <td>{items.CuisineName}</td>
                                                            <td><input type="text" id={items.PushId} value={items.Details} onChange={onChangeDetailsHandler} className="price" /></td>
                                                            <td><Button type="button" className="warning" id={items.PushId} onClick={onSaveDetailsHandler}><Check id={items.PushId} onClick={onSaveDetailsHandler} size={20} /></Button></td>                                                            {Object.keys(items.Portions).length === 0 ?
                                                                <td>{""}</td> :
                                                                <>
                                                                    {Object.keys(items.Portions).map((item) => (
                                                                        <ul >
                                                                            <li >{item}:{items.Portions[item]}</li>
                                                                        </ul>
                                                                    ))}
                                                                </>
                                                            }
                                                            ...
                                                            <td><Button type="button" className="warning" id={items.PushId} onClick={onSaveHandler}><Check id={items.PushId} onClick={onSaveHandler} size={20} /></Button></td>
                                                            <td>{items.STime}</td>
                                                            <td>{items.ETime}</td>
                                                                ...
                                                                <tr key={id}>
                                                                    {items !== undefined ?
                                                                        <>
                                                                            ...
                                                                            <td><Button type="button" className="warning" id={items.PushId} onClick={onOfferSaveHandler} ><Check id={items.PushId} onClick={onOfferSaveHandler} size={20} /></Button></td>
                                                                            <td>{items.STime}</td>
                                                                            <td>{items.ETime}</td>
                                                                            {items.Status === "Active" ?
                                                                                <td><div style={{ position: "relative", maxWidth: "50%", maxHeight: "50%" }}>
