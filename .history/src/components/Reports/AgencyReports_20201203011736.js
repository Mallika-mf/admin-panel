...
const AgencyReports = (props) => {
  ...
  const [filterfn,setFilterFn] = useState({fn : items=> {return items;}})
    ...
    const [agencyShow,setAgencyShow] = useState([])
    useEffect(()=>{
        try{
            ...
            database.ref().child("Agency").orderByChild("Created").equalTo(today)
...
const recordsAfterDateShow = () => {
  return filterfn.fn(agencyShow).slice(page * rowsPerPage, (page + 1) * rowsPerPage)
}
       return (
        <Fragment>
