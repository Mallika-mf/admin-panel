...
app.database().ref().child('-M9w7vN_aAU0vUc75x9A').orderByChild('Status').equalTo('Active').on('value',snapshot=>{
  let content = []
  let citysnap
 ...
 const printDocument=(event)=> {  
  ...
  console.log(userid)
    app.database().ref().child("-M9w7vN_aAU0vUc75x9A").child(userid).child("Status").set("InActive")
