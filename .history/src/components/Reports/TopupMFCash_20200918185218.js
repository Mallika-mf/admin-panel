...
const onSubmit=(event)=>{
    ...
    if (person == password) {
        ...
        firebaseref1.transaction(function(currentstock) {
           ...
           return currentstock;
             ...
             function(error, committed, snapshot) {
                a=snapshot.val();
                console.log(a);
                var ref=app.database().ref().child("CloudKitchen").child(sname).child("CoinsTransactions").push();
                ...
                .once('value').then(function(snapshot) {
                    ...
                    snapshot.forEach(function(data){
                        ...
                        if(val.UserId!=""&&val.UserId!=null){
                            ...
                            window.cno.push(val.MobileNumber);
                            window.balance.push(val.Coins);
                        }
                    });
                    // document.getElementById("#sname").autocomplete({
                    //     source: window.cid
       ...
       return (
        <Fragment>
            <BreadCrumb parent={<Home/>} subparent="Reports" title="Topup MF Cash"/>
                ...
                <Row>
                    <Col sm="12">
                     ...
                     <Input type="text" className="form-control" id="sname" value={sname} onChange={onChangeIdHandler}  autoComplete={cID} placeholder="Chef ID"/>
                        <p style={{color:"red",margin: "1%"}} id="balance">MF Cash Balance : </p>
                         </div>
                        <div className="col-sm-1 col-md-2">
                        <span id="search" onClick={onChangeSearchHandler}><img src="https://img.icons8.com/ios-filled/24/000000/search.png"/></span>
                         </div>
                         </Row> 
