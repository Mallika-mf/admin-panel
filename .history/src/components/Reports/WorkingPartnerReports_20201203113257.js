...
const onUpdateChef=(event)=>{
  ...
  .orderByChild("Franchise").equalTo(event.target.id)
  .once("value", function(snapshot) {
    if(snapshot.exists()){
        app.database().ref().child("Franchise").child(event.target.id).child("Chefs").set(parseInt(snapshot.numChildren()));

    }else{
              app.database().ref().child("Franchise").child(event.target.id).child("Chefs").set(parseInt(0));
