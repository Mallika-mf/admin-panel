...
const ChefApprovalChange = () => {
  ...
  const [show,setShow] = useState(true)
    ...
    useEffect(()=>{
        try{
            ...
            database.ref().child("ApprovalRequests")
  ...
  const idClickHandler=(event)=>{
       ...
       history.push(`${process.env.PUBLIC_URL}/approvals/chef-approvals`);
    
  }

  const CloseHandler=(event)=>{
    var userid=event.target.id    
    var firebaseref=app.database().ref().child("ApprovalRequests").child(userid).remove();
 ...
 const printDocument=(event)=> {  
    ...
    html2canvas(input)  
      .then((canvas) => {  
        ...
        pdf.save("chefApprovalChangeReport.pdf");  
      });  
  }
       ...
       return (
        <Fragment>
            ...
            <Container fluid={true}>
                ...
                table="datatable"  
                filename="chefApprovalChangeReport"  
                sheet="chefApprovalChangeReport"  
                buttonText="Excel" />
                <iframe
                    id="iDatatable"
                    src="/reports/approval-change-report"
                    style={{ display: 'none' }}
                    title="Receipt"
                />
...
export default ChefApprovalChange;