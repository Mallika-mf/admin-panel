...
const ChefDescriptions = () => {
        ...
        .once('value', function(snapshot){
          ...
          if(snapshot.exists()){
            ...
            snapshot.forEach(snap=>{
              setCityStatus(snap.val().City)
              setLocalFood(snap.val().Local)
  ...
  const onSaveHandler=(event)=>{
    ...
    if(localFood==="Yes"){
      app.database().ref().child("LocalFood").child(cityStatus).child(userid).child("Brand").set(brands)
      app.database().ref().child("LocalFood").child(cityStatus).child(userid).child("Details").set(details)
    }else{
      app.database().ref().child(cityStatus).child(userid).child("Brand").set(brands)
      app.database().ref().child(cityStatus).child(userid).child("Details").set(details)
    }
                                ...
                                <Table id="datatable">
                                    ...
                                    {users.filter(order=>order.UserId.includes(search)).map((item,id)=>{
                                                    return(
                                                       ...
                                                       <td style={{textAlign:"center", fontSize: "25px", fontWeight: "bold"}}><button type="button" id={item.UserId} onClick={onSaveHandler}  className="btn btn-success btn-md">{"Save"}</button></td>
