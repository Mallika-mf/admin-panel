...
const ChefInActiveReport = () => {
  ...
  const onClickDeleteHandler = (event) => {
    ...
    if (superadmin == "Yes") {
      Swal.fire({
        ...
        .then((willDelete) => {
          if (willDelete.value) {
            app.database().ref().child("CloudKitchenInActive").child(localityId).remove();
            Swal.fire({
              icon: "success",
              text: "Deleted!"
            });
  ...
  const onChangePassPort = (event) => {
    ...
    const uploadTask = storage.ref(`CloudKitchenInActive/${name}`).put(image,metadata)
    uploadTask.on('state_changed',
      (snapShot) => {
        ...
        storage.ref("CloudKitchenInActive/").child(name).getDownloadURL()
          .then(fireBaseUrl => {
  ...
  const onClickSearchHandler = (event) => {
    ...
    var firebaseref = app.database().ref().child("CloudKitchenInActive").child(id);
  ...
  const onUpdateHandler = (event) => {
    ...
    var firebaseref = app.database().ref().child("CloudKitchenInActive").child(sname);
    ...
    cno = [];
    cid = [];
    app.database().ref().child("CloudKitchenInActive")
      .once('value').then(function (snapshot) {
  ...
  const onChangeFstatus = (event) => {
    ...
    if (event.target.value === "Select") {
      ...
      var database = app.database();
      database.ref().child("CloudKitchenInActive").orderByChild("AStatus").equalTo(event.target.value)
        .once('value', function (snapshot) {
          if (snapshot.exists()) {
            // $('#datatable').empty();
            var content = [];
  ...
  const onChangeSCityHandler = (event) => {
    ...
    if (event.target.value == "Select") {
      ...
      var database = app.database();
      database.ref().child("CloudKitchenInActive").orderByChild("City").equalTo(event.target.value)
        .once('value', function (snapshot) {
          if (snapshot.exists()) {
            // $('#datatable').empty();
            var content = [];
  ...
  const onChangeFtaHandler = (event) => {
    ...
    if (event.target.value == "Select") {
      ...
      var database = app.database();
      database.ref().child("CloudKitchenInActive").orderByChild("Passed").equalTo(event.target.value)
        .once('value', function (snapshot) {
          if (snapshot.exists()) {
            // $('#datatable').empty();
            var content = [];
  ...
  const onChangeFcaHandler = (event) => {
    ...
    if (event.target.value == "Select") {
      ...
      var database = app.database();
      database.ref().child("CloudKitchenInActive").orderByChild("Catering").equalTo(event.target.value)
        .once('value', function (snapshot) {
          if (snapshot.exists()) {
            // $('#datatable').empty();
            var content = [];
...
//     } else {
//       var database = app.database();
//       database.ref().child("CloudKitchenInActive").orderByChild("Paid").equalTo(event.target.value)
//         .once('value', function (snapshot) {
//           if (snapshot.exists()) {
//             // $('#datatable').empty();
//             var content = [];
...
//   }
  ...
  const onChangeSdate = (event) => {
    setSdate(event.target.value)
    var database = app.database();
    database.ref().child("CloudKitchenInActive").orderByChild("Created").equalTo(event.target.value)
      .once('value', function (snapshot) {
        if (snapshot.exists()) {
          // $('#datatable').empty();
          var content = [];
  ...
  const printDocument = (event) => {
    ...
    html2canvas(input)
      .then((canvas) => {
        ...
        pdf.save("ChefInActiveReport.pdf");
      });
  }
                        ...
                        <Col className="col-md-6 text-right">
                          ...
                          <div className="dt-buttons btn-group">
                            ...
                            <ReactHTMLTableToExcel
                              ...
                              table="datatable"
                              filename="ChefInActiveReport"
                              sheet="ChefInActiveReport"
                              buttonText="Excel" />
                            <iframe
                              id="iDatatable"
                          ...
                          {recordsAfterPagingAndSorting().map((item, id) => {
                            return (
                              <TableRow key={id}>
                                ...
                                <TableCell className="">{item.Membership}</TableCell>
  ...
  export default ChefInActiveReport;