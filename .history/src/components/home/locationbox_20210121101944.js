...
const LocationBox = (props) => {
    ...
    const getCoordinates = (position) => {
        ...
        .then(response => {
            localStorage.setItem('formattedAddress', response.results[0].formatted_address)
            setLocation({
    ...
    const changeLocation = async(event)=>{
        ...
        for(let city of allCities.cities){
            if(userlocation.query.includes('Secunderabad')){
                ...
                cityInfo = allCities.cities.find(city => city.PushId === '-M7Rt3hrXVXrmglMhgMb');
                // setAppLocation(cityInfo);
            ...
            }else if(userlocation.query.includes('Delhi')|| userlocation.query.includes('Gurugram')||userlocation.query.includes('New Delhi')
                    ||userlocation.query.includes('Noida')|| userlocation.query.includes('Ghaziabad')
                    || userlocation.query.includes('Faridabad')||userlocation.query.includes('gurgaon')){
                        localStorage.setItem('cityname', 'Delhi');
                        localStorage.setItem('pushid', '-M9w7qfvWoy4sRUCqcGQ');
                        localStorage.setItem('radius', '5');
                        cityInfo = allCities.cities.find(city => city.PushId === '-M9w7qfvWoy4sRUCqcGQ');
                        // setAppLocation(cityInfo);
                        await Geocode.fromAddress(userlocation.query)
                        .then((response) => {
                            const { lat, lng } = response.results[0].geometry.location;
            ...
            if(userlocation.query.includes(city.Name)){
                cityInfo = city;
                localStorage.setItem('cityname',city.Name);
                localStorage.setItem('pushid',city.PushId);
                localStorage.setItem('radius',city.Radius);
...
======= */}
                      ...
                      <Form.Group className="col-lg-7 col-md-7 col-sm-12">
                        ...
                        {error?
                    <div className="card" style={{height:"40px",backgroundColor:"pink"}}>{error}</div>:null}
                      </Form.Group>
                      <Form.Group className="col-lg-2 col-md-2 col-sm-12">
