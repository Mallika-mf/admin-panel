...
import {Row,Col,Container,Accordion,Button,Form,Spinner} from 'react-bootstrap';
			         ...
			         <Container>
			            <Row className="d-none-m">
			               <Col md={12}>
								 ...
								 <div className="product-sort d-flex justify-content-between align-items-center float-right" style={{marginBottom:"2%"}} >
                  <div className="sort d-flex align-items-center" variant="outline-info">
                    <span className="m-right-15 ">Sort By: </span>
                    <div className="form-group" style={{maxWidth:"100%",width:"200px"}}>
                      <div className="select-basic">
							  ...
							  this.paginate(this.state.page).map((item,index) => {
								  ...
								  if(item.Cuisines) {
									Object.keys(item.Cuisines).forEach(function(key, index) {
									  if(index < 3) {
										cuisinesitems.push( < >&nbsp;{(index > 0) ? "•" : ''}&nbsp;{item.Cuisines[key].Name}</>)
									  }
									})
								  }
                        if(availableChefs.find(chef => chef.UserId === item.UserId)) {
                          ...
                          return(
			                     <Col md={4}  sm={6} key = {index} className="mb-4 pb-2">
			                        <CardItem 
										...
										time={item.DeliveryTime? item.DeliveryTime + " mins": "40 mins"}
										price={item.CostTwo? item.CostTwo + " For Two": "250 For Two"}
										showPromoted={true}
										promotedVariant='dark'
										favIcoIconColor={item.Veg === "Yes"?"text-green":"text-danger"}
										rating={item.Ratings !== undefined? Math.trunc(item.Ratings*10)/10: "5"}
										available={false}
										product={item}
										id={"orderdetails/" + item.UserId}
                          ...
                          return(
			                     <Col md={4} sm={6} className="mb-4 pb-2" key={index} style={{ border: "none", opacity: '0.7' }}>
			                        <CardItem 
								   		title={item.Name}
										   subTitle={cuisinesitems}
										   imageAlt='Product'
