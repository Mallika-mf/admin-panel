...
class Detail extends React.Component {
	...
	componentDidMount() {
		...
		//   this.loadUserCart();
		firebase.database().ref().child("User").child(userName).child("Cart")
		.on("value",snapshot=>{
			let content = []
			snapshot.forEach(snap=>{
				let num = 0
				num = num+parseInt(snap.val().Total)
				content.push(num)
			})
			console.log(content)
		},()=>{})
							...
							<Row className="d-flex align-items-end">
								<Col md={8}>
									<div className="restaurant-detailed-header-left">
										...
										<h2 className="text-white">{prd.Name}</h2>
										<h6 className="text-white mb-1"><Icofont icon="icofont-home" /> {prd.KitchenName}<Badge variant="success">OPEN</Badge>
										</h6>

										<p className="text-white mb-0"><Icofont icon="food-cart" /> {cuisinesitems}
										</p>
									</div>
								</Col>
								<Col md={4}>
						  ...
						  return(
							  <>
										...
										<div className="mb-2 bg-white rounded p-2 clearfix">
											...
											<h6 className="font-weight-bold text-right mb-2">Subtotal : <span className="text-danger">{userCart.Price}</span></h6>
