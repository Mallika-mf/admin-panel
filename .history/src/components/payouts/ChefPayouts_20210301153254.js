                        ...
                        data.child('Transactions').forEach(function (data1) {
                            ...
                            if (val1.Status === "Pending") {
                                if (val1.Date >= state.sdate && val1.Date <= state.edate) {
                                    set += 1
                                    val.totalOrder = set
                                    if(val1.TransactionType==="Cr"){
                                        amount = amount + Math.trunc(val1.Amount*10)/10
                                    }else{
                                        amount = amount - Math.trunc(val1.Amount*10)/10
                                    }
            ...
            .once('value', function (snapshot) {
                if (snapshot.exists()) {
                    ...
                    snapshot.forEach(function (data) {
                        ...
                        if (val.Status === "Pending") {
                            if (val.Date >= state.sdate && val.Date <= state.edate) {
                                if(val.TransactionType==="Cr"){
                                    amount = amount + Math.trunc(val.Amount*10)/10;
                                }else{
                                    amount = amount - Math.trunc(val.Amount*10)/10;
                                                ...
                                                {recordsAfterPagingAndSorting().map((item, id) => {
                                                    return (
                                                        <tr key={id}>
                                                            ...
                                                            <td className="actions" style={{ textAlign: "center" }}><Link to="/setting/partial-refund"><Button>Deduct Money</Button></Link></td>
