...
//End Template State
	loadUserCart = async () => {
		...
		if (user.exists()) {
		  ...
		  console.log(Object.values(get(user.val()[userName], 'Cart', [])))
		}
	  }
	  ...
	  loadVendors = async chef => {
		...
		try {
		  ...
		  if (vendor.exists()) {
			...
			let userCart = []
			for (var j of this.state.userCart) {
				userCart.push(j);
			}
			userCart.push(this.state.userCart);
			...
			const foodItemsWithQuantity = foodItems.map(obj => {
			  let cartItemMatch = userCart.find(userCartItem => {
				  console.log(userCartItem.PushId)
				  console.log(obj.PushId)
				(userCartItem.PushId === obj.PushId)})
