				...
				<Container>
									...
									{get(userAddresses, 'length', 0) > 0 ?
										   ...
										   userAddresses.map((userAddress, index) => {
											   let currentRedius = localStorage.getItem("radius");
														...
														this.getUserRadiusBetweenTwoLocations1(String(userAddress.Coord))  
													   if (this.getUserRadiusBetweenTwoLocations1(String(userAddress.Coord)) < currentRedius) {
										...
										<Col md={6} key={index}>
											<ChooseAddressCard
												...
												onDeliverHereClick={() => this.handleAddressSelect(userAddress)}
											/>
										</Col>
													   )}
													})
															...
															<div className="form-row">
																<Form.Group className="col-md-12">
																	...
																	<InputGroup>
																		...
																		<InputGroup.Append>
																			<Button variant="outline-secondary" type="button" id="button-addon1"><Icofont icon="card" /></Button>
																		</InputGroup.Append>
																	</InputGroup>
																</Form.Group>
							...
							<div className="generator-bg rounded shadow-sm mb-4 p-4 osahan-cart-item">
								...
								{this.state.showPromocode === true ?
									...
									<InputGroup className="input-group-sm mb-2">
										...
										<div className="input-group-append" onClick={this.handleRemovePromo} id="promocodeButton" style={{ "cursor": "pointer" }} disbaled={!this.state.removeDiscount.length === 0}>
                  ...
                  >
                    {this.state.promocodeDisplay &&
                      ...
                      this.state.promocodeDisplay.map((item,id)=>{
                        if(item.Type!=="Delivery"){
                            return(
                                <div className="row"  >
                    <div className="col-12 col-sm-12 col-md-12 col-lg-12 text-left" style={{display:"flex"}}>
                            <div  style={{borderStyle:"dotted",marginTop:"5%",width:"55px",height:"80px",borderColor:"gray",lineHeight:"18px",marginLeft:"4%"}}><strong style={{fontWeight:600}}>Get</strong><br/><span style={{marginTop:"-10%",color:"black",marginLeft:"2%",fontWeight:600}}>Up to</span><br/><span style={{marginTop:"-10%",color:"black",marginLeft:"2%",fontWeight:600}}>{item.Discount}%</span><br/><small  style={{color:"green",fontWeight:650}}>Discount</small></div>
                            <div className="col-md-8 " style={{width:"355px",height:"80px",marginTop:"-65px",wordBreak:"word-break",marginLeft:"70px",textAlign:"justify",position:"relative",lineHeight:"12px"}}><span style={{fontSize:"15px",color:"red",fontWeight:800}}>{item.Name}</span><br/><small style={{fontSize:"11px",fontWeight:500}}>get upto {item.Discount}% discount for order value above ₹{item.MinAmount}<br/>Maximun Discount ₹{item.MaxAmount}</small></div>
                    ...
                    <div className="col-12 col-sm-12 col-md-12 col-lg-12 text-left">
                            <div  style={{borderStyle:"dotted",marginTop:"5%",width:"55px",height:"80px",borderColor:"gray",lineHeight:"18px",marginLeft:"4%"}}><strong style={{fontWeight:600}}>Get</strong><br/><span style={{marginTop:"-10%",color:"black",marginLeft:"2%",fontWeight:600}}>Up to</span><br/><span style={{marginTop:"-10%",color:"black",marginLeft:"2%",fontWeight:600}}>{item.MaxAmount}%</span><br/><small  style={{color:"green",fontWeight:650}}>Discount</small></div>
                            <div className="col-md-8 " style={{width:"355px",height:"80px",marginTop:"-65px",wordBreak:"word-break",marginLeft:"70px",textAlign:"justify",position:"relative",lineHeight:"12px"}}><span style={{fontSize:"15px",color:"red",fontWeight:800}}>{item.Name}</span><br/><small style={{fontSize:"11px",fontWeight:500}}>get free Delivery for  for order value<br/> above ₹{item.MinAmount}</small></div>
                        {/* <img src="../assets/img/jumbo.png" /> */}
                    </div>
                </div>
                            )
								...
								<div className="mb-2 bg-white rounded p-2 clearfix">
									<p className="mb-1" style={{ textAlign: "justify" }}>Sub Total <span className="float-right text-dark">₹ {subTotal ? subTotal.toFixed(2) : 0.00}</span></p>
									<p className="mb-1" style={{ textAlign: "justify" }}>Delivery Fee
                    		<OverlayTrigger
											key="top"
											placement="top"
											overlay={
										...
										<span className="float-right text-dark">₹ {deliveryCharges ? parseFloat(deliveryCharges).toFixed(2) : 0.00}</span>

									</p>
									<p className="mb-1" style={{ textAlign: "justify" }}>Packing Charges <span className="float-right text-dark">₹ {packingCharges ? parseFloat(packingCharges).toFixed(2) : 0.00}</span></p>

									<p className="mb-1 text-success" style={{ textAlign: "justify" }}>Total Discount <span className="float-right text-success"> ₹ {discount1 ? parseFloat(discount1).toFixed(2) : 0.00}</span></p>
									<p className="mb-1 text-dark" style={{ textAlign: "justify" }}>Taxes &amp; Charges  <span className="float-right text-dark">₹ {taxes ? parseFloat(taxes).toFixed(2) : 0.00}</span></p>

									<hr />
									<h6 className="font-weight-bold mb-0" style={{ textAlign: "justify" }}>Total Bill:<br />Taxes &amp; Charges Included  <span className="float-right">₹ {total ? total.toFixed(2) : 0.00}</span></h6>
								</div>
								<Button onClick={this.handlePlaceOrder} className="btn btn-success btn-block btn-lg">PAY ₹ {total ? total.toFixed(2) : 0.00}
