...
import {Container,Row,Col,Card,CardHeader,Table,Button,Modal,FormGroup,ModalBody,ModalFooter,ModalHeader} from "reactstrap";
   ...
   await app.database().ref().child("Masters").child("Localities")
    ...
    const  onChangeHandler=(event)=>{
       ...
       const onClickRefundHandler=(event)=>{
        ...
        var total = arrData[2]
                   Swal.fire({
            title: "Refund through My Cash or Original Wallet",
            text: "",
            icon: "warning",
            showDenyButton: true,
            showCancelButton: true,
            confirmButtonText: 'My Cash',
            denyButtonText: `Original Wallet`,
          ...
          .then((result) => {
            if (result.isConfirmed) {
                app.database().ref().child("Users").child(userid).once('value').then(function(snapshot){
                    ...
                    console.log(val.WalletInsta)
                    if(val.WalletInsta===undefined){
        ...
        console.log(pushId)
         currentdate = new Date(); 
          datetime = currentdate.getDate() + "/"
                              ...
                              + currentdate.getMinutes() + ":" 
                              + currentdate.getSeconds();
         ref = app.database().ref().child("Orders").child(pushId)
        ref.child("Status").set("100");
        ref.child("RefundDateTime").set(datetime)
                                ...
                                <table id="datatable"  className="table table-striped "  >
                                    ...
                                    return(
                                       ...
                                       <td className="actions-view" style={{textAlign:"center"}}><button type="button" id={item.Pushid} onClick={onClickViewItemHandler} className="btn btn-primary btn-md" data-toggle="modal" data-target="#myModal">{"View"}</button></td>
                                        ...
                                        {item.UserCancel==="Yes"?
                                          <td>{item.CName}:{item.Reason}</td>: item.ChefCancel==="Yes"?
                                          <td>{item.Chef}:{item.Reason}</td>:
                                          <td>{""}</td>
