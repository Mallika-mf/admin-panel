...
const AddFoodItems = () => {
    ...
    return firebaseref.on('value').then(function (snapshot) {
      if (snapshot.exists()) {
        ...
        firebaseref.child("ADDONS").on('value').then(function (snap){
            if(snap.exists()){
                let content  = []
                snap.forEach(data1=>{
                   let val = data1.val()
        ...
        firebaseref.child("FOODTYPE").on('value').then(function (snap){
            if(snap.exists()){
                let content  = []
                snap.forEach(data1=>{
                   let val = data1.val()
    ...
    <tbody>
        {addonList.map((item,id)=>{
            ...
            <tr key = {id}>
                ...
                {/* {cityName[cityPushId.indexOf(item.ChefCity)]} */}
        <td>{item.pushid}</td>
            </tr>
            )
        })}
