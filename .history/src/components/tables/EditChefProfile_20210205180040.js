...
const onClickCuisineDelete = (event,index) =>{
  cuisineList.map(item=>{
    if(event.target.id===item.PushId && event.target.id!==undefined){
      app.database().ref().child('CloudKitchen').child(search).child('Cuisines').child(event.target.id).remove()
    }else{
      const newData = cuisineList.filter((_, item) => {
        return (
          item !== index
        )
    
      })
      setCuisineList(newData)
    }
  })
...
const onDeleteFoodType = (event,index) =>{
  foodTypeList.map(item=>{
    if(event.target.id===item.PushId && event.target.id!==undefined){
      app.database().ref().child('CloudKitchen').child(search).child('FoodType').child(event.target.id).remove()
    }else{
  ...
  //  app.database().ref().child('CloudKitchen').child(search).child('AStatus').set('InActive')
...
for (var index = 0; index < cuisineList.length; index++) {
  ...
  if(locality===true){
    firebaseref.child("Location").set(markerPosition.lat+","+markerPosition.lng)
    firebaseref.child("LocationSet").set("Yes")
...
let map;
 if( center.lat !== undefined ) {
  map = <div>
    ...
    <AsyncMap
     googleMapURL="https://maps.googleapis.com/maps/api/js?key=AIzaSyCPhxfpptoIc1yca5U8mXIigIajoERQCdE&libraries=places"
     loadingElement={
      ...
      <Container fluid={true}>
        ...
        <Row className="form-row">
          <Col className="form-group col-md-3">
            ...
            <input type="text" value={deliveryTime} onChange={onChangeDeliveryTime} className="form-control"/>
            <div className="clearfix"></div>
          </Col>
        </Row>
        <Row className="form-row">
              ...
              {cuisineList.map((item, index) => {
                return (
                  <tr key={index}>
                    <td>{item.Name}</td>
                    {/* <td>{}</td> */}
                    <td><Trash id={item.PushId} onClick={(event)=>onClickCuisineDelete(event,index)} size={15} /></td>
                  </tr>
                )
              })}
            </tbody>
            ...
            <tbody>
              {foodTypeList.map((item, index) => {
                return (
                  <tr key={index}>
                    <td>{item.Name}</td>
                    <td><Trash id={item.PushId} onClick={(event)=> onDeleteFoodType(event,index)} size={15} /></td>
                  </tr>
                )
              })}
            </tbody>
