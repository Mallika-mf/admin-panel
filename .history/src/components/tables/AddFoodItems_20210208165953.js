...
const AddFoodItems = (props) => {
  ...
  const onChangeFoodName = (event) => {
    ...
    const filterSuggestion = searchValue.filter(suggestion=>suggestion.Name.toLowerCase().indexOf(event.target.value.toLowerCase() > -1))
    setActiveSuggestions(0)
    setFilterSuggestion(filterSuggestion)
  ...
  const onClickFoodItems = (event)=>{
    ...
    var firebaseref = app.database().ref().child("CloudKitchen").child(search).child("FoodItems").child(event.target.innerText);
...
if (showSuggestion && foodName) {
  if (filterSuggestion.length) {
    suggestionListComponent = (
      <ul className="suggestions">{console.log(filterSuggestion)}
        {filterSuggestion.map((suggestion, index) => {
          ...
          return (
            <li className={className} key={suggestion.PushId} onClick={onClickFoodItems}>
              {suggestion.Name}
            </li>
          );
        })}
  ...
  const onClickAddonDelete = (event,index) => {
    ...
    addonList.map(item=>{
      if(event.target.id===item.data && event.target.id!==undefined){
        app.database().ref().child("CloudKitchen").child(search).child("FoodItems").child(foodName).child("ADDONS").child(event.target.id).remove();
      }else{
  ...
  const onDeleteFoodType = (event,index) => {
    foodTypeList.map(item=>{
      if(event.target.id===item.data && event.target.id!==undefined){
        app.database().ref().child("CloudKitchen").child(search).child("FoodItems").child(foodName).child("FOODTYPE").child(event.target.id).remove();
  ...
  const onClickPortionDelete = (event,index) => {
    ...
    portionList.map(item=>{
      if(event.target.id===item.data && event.target.id!==undefined){
       app.database().ref().child("CloudKitchen").child(search).child("FoodItems").child(foodName).child("Portions").child(event.target.id).remove()
