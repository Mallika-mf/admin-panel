  ...
  const onClickSearchHandler = (event) => {
    ...
    return firebaseref.once('value').then(function (snapshot) {
      if (snapshot.exists()) {
        ...
        if (snapshot.val().Locality !== undefined) {
          var mLocalityList = [];
          var database = app.database();
          database.ref().child("Masters").child("Localities")
    ...
    stock.transaction(function (currentstock) {
      ...
      function (error, committed, snapshot) {
        if (error) {
          ...
          firebaseref.child("CreatedBy").set("WebUser")
          ...
          <Col sm="12">
            ...
            <Row className="form-row">
              <Col className="form-group col-md-6">
                ...
                <Input type="number" value={mnumber} onChange={onChangeMobileNumber} className="form-control" placeholder="Mobile Number" />
                <div className="clearfix"></div>
              </Col>
              {hideSignIn !== false ?
                <Col className="form-group col-md-2" >
