...
import { Table, TableBody, TableCell, TableRow, TableHead, makeStyles, TablePagination, Toolbar } from '@material-ui/core'
...
const ChefListTable = () => {
        ...
        if (target === "") {
          ...
          return items.filter(x => {
            return Object.keys(x).some(key =>
              searchValue.includes(key) ? false : x[key].toString().toLowerCase().includes(target)
            )
          })
  ...
  const onClickRecommendationhHandler=(event)=>{
    console.log(event.target.id)
    app.database().ref().child("CloudKitchen").child(event.target.id).child("Highlight").set(event.target.value)
  }
            ...
            <Row>
              ...
              <Col sm="12">
                <Card>
                  ...
                  <div className="table-responsive text-nowrap table align-items-center table-flush table-striped ">
                    <Table id="datatable" className={classes.table} >
                      <TableHead>
                        <TableRow>
                          ...
                          <TableCell scope="col">Delete Photo</TableCell>
                          <TableCell scope="col">Recommendation</TableCell>
                          ...
                          return (
                            <TableRow key={id}>
                              ...
                              {item.UserId === "undefined" ?
                                ...
                                <>
                                  <TableCell><a href={item.PP} target="_blank" rel="noopener noreferrer">View</a></TableCell>
                                  <TableCell ><Button type="button"><Trash id={item.UserId} onClick={deletePhotoHandler} size={15} /></Button></TableCell>

                                </>
                              }
                              <TableCell><select  id={item.UserId} value={item.Highlight} onChange={onClickRecommendationhHandler} className="btn btn-success btn-md">
                              ...
                              <TableCell><button type="button" id={item.UserId} onClick={onClickSearchHandler} className="btn btn-success btn-md">{"View"}</button></TableCell>
                              <TableCell><button type="button" id={item.UserId} onClick={onClickDeleteHandler} className="btn btn-danger btn-md">{"Delete"}</button></TableCell>
