...
const AddFoodItems = () => {
  ...
  const onSubmitHandler = (event) => {
    ...
    for (var foodIndex = 0; foodIndex < foodTypeList.length; index++) {


      firebaseref.child("FOODTYPE").child(foodTypeList[foodIndex].data).set(foodTypeList[foodIndex].data)
  ...
  const onUpdateHandler = (event) => {
    ...
    firebaseref.child("Category").set(veg)
    firebaseref.child("FoodType").set(foodCategory)
      //  firebaseref.child("ADDONS").child(addonList.data).set(addonList.price)
       console.log(addonList[data])
       console.log(addonList[price])
