...
const onChangeCuisine = (event) =>{
    ...
    setSelectCuisineId([...cuisineList,lockerId])
...
const onChangeFoodCategory = (event) =>{
    ...
    let lockerId = event.target.value
    setSelectFoodTypeId(...foodTypeList, lockerId)
}
...
for (var index = 0; index < cuisineList.length; index++) {
    ...
    firebaseref.child("Cuisine").child(cuisineList[index].PushId).child("Name").set(cuisineList[index].Name)
    firebaseref.child("Cuisine").child(cuisineList[index].PushId).child("PushId").set(cuisineList[index].PushId)
  ...
  firebaseref.child("CuisineList").set(selectCuisineId)
