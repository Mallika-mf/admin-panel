...
const WaitingPickup = () => {
    ...
    const onClickUber = (event) =>{
        setUserPushId(event.target.id)
        setUber(true)
    }
    const onShowModal2 = () => { setUber(false) }

    const onChangeDriverName = (event) => {
        setDriverName(event.target.value);
    }
    const onChangeDriverNumber = (event) => {
        setDriverNumber(event.target.value);
    }

    const onSubmitUber = (event) =>{
        ...
        app.database().ref().child("Orders").child(userPushId).child("DeliveryNumber").set(driverNumber)
        app.database().ref().child("Orders").child(userPushId).child("DeliveryName").set(driverName)
        Swal.fire({
        ...
        Swal.fire({
            ...
            .then((willDelete) => {
                if (willDelete.value) {
                   ...
                   axios({
                    method: 'get',
                    url: "https://chotabeta.app/dev/testenv/api/vendor-order-cancellation",
                    
                     headers:{"apikey":"xNupeGn1qBtIFnD7dGJxc8T9gSXDgeMj",},
                     data: {
                        "order_id": pushId,
                    "reason": "Driver Delay"
                    }
                })
                //    axios.post("https://clb.rapido.bike/delivery/client/order/cancel",param,{
                //        "headers":{
                //         // "Content-Type": "application/json",
                //         "apikey":"xNupeGn1qBtIFnD7dGJxc8T9gSXDgeMj",
                ...
                //    })
                   .then(response =>{console.log(response)
                    if(response.data.info["status"]==="error"){
                                   ...
                                   app.database().ref("/Orders/"+userid).child("DeliveryApiRemarks").set("Rapido : "+response.data.info["message"]);
    ...
    const onClickPickkup= (event) => {
        var pushId = event.target.id
        Swal.fire({
            title: "Are you sure want to reassign the Pickkup?",
            text: "",
            icon: "warning",
            showCancelButton: true,
            confirmButtonText: 'OK',
            ...
            .then((willDelete) => {
                if (willDelete.value) {
                    ...
                    firebaseref.child("DeliveryName").remove();
                    firebaseref.child("DeliveryNumber").remove();
                    firebaseref.child("Pickkup").remove();

                    app.database().ref().child("Orders").child(pushId).child("Pickkup").set("Yes");
                    Swal.fire({
                        title: "Reassigned Pickkup Successfully!",
                        text: "",
                        icon: "success",
                    });
                }
    ...
    const onCancelChotabeta = (event) =>{
        var arr  = event.target.id.split("+") 
        var chotabetaId = arr[0]
        var pushid = arr[1]
        console.log(arr)
        console.log(chotabetaId)
        Swal.fire({
            title: "Are you sure want to cancel the delivery boy?",
            text: "",
            icon: "warning",
            ...
            .then((willDelete) => {
                if (willDelete.value) {
                    ...
                    // axios({
                    //     // method: 'get',
                    //     url: "https://stackroger.com/api/vendor-order-cancellation",
                        
                    //      headers:{"Content-Type": "application/json"},
                    //      data: {
                    //         "order_id": chotabetaId,
                    ...
                    axios.get(`https://cors-anywhere.herokuapp.com/https://stackroger.com/api/vendor-order-cancellation/`,param,{
                        "headers":{
                   ...
                   .then(response =>{console.log(response)
                    app.database().ref().child("Orders").child(pushid).child("DeliveryApiRemarks").set(response.data.message)
                })
        ...
        Swal.fire({
            ...
            .then((willDelete) => {
                if (willDelete.value) {
                    ...
                    // })
                   .then(response =>{console.log(response)
                    app.database().ref().child("Orders").child(pushid).child("DeliveryApiRemarks").set(response.data.status)
                })
                   .catch(function (response) {
                    //handle error
                    console.log(response.message);
        ...
        Swal.fire({
            ...
            .then((willDelete) => {
                if (willDelete.value) {
                    ...
                    axios.get(`https://ops.pickkup.com/api/cancel_order?apiKey=nq1fZTkOoKM@CUL2XJV@adSyCxU$cI3nTb2McTonAO&orderId=${pushId}`,{
                        "headers":{
                            "Content-Type" : "application/json",
                   ...
                   .then(response =>{
                    ...
                    app.database().ref().child("Orders").child(orderid).child("DeliveryApiRemarks").set(response.data.message)
                   }
                                            ...
                                            return (
                                                <tr key={id}>
                                                    ...
                                                    {item.DeliveryApiRemarks !== undefined && item.PickUpETA !== undefined && item.DropETA !== undefined && item.DunzoTrackingUrl !== undefined ?
                                                     ...
                                                     <td className="actions-view" style={{ textAlign: "center" }}><button type="button" id={item.Pushid} onClick={onClickChotabeta} className="btn btn-primary btn-md">{"Reassign"}</button><br/>
                                                     <Button id={item.ChotabetaOrderId+"+"+item.Pushid} className="warning" onClick={onCancelChotabeta}>Cancel</Button></td>
                                                    ...
                                                    <Button id={item.RapidoOrderId+"+"+item.Pushid} className="warning" onClick={onCancelRapido}>Cancel</Button></td>
                                                    ...
                                                    <td className="actions-view" style={{ textAlign: "center" }}><button type="button" id={item.Pushid} onClick={onClickPickkup} className="btn btn-primary btn-md">{"Reassign"}</button><br/><br/>
                                                    <Button type="button"id={item.PickkupOrderId+"+"+item.Pushid} onClick={onCancelPickkup}  className="warning">{"Cancel"}</Button></td>

                                                    <td className="actions-view" style={{ textAlign: "center" }}><button type="button" id={item.Pushid} onClick={onClickUber} className="btn btn-primary btn-md">{"Reassign"}</button></td>
  ...
  <p style={{fontSize: "15px",marginLeft:"6%"}}><b>{chefDescription}</b></p>
                            ...
                            <input type="text" value={driverName} required="" onChange={onChangeDriverName}  className="form-control" placeholder="Enter Name...." />
                            <div className="clearfix"></div>
                        </div>
                                <div className="form-group col-md-9">
                            <label className="form-label">Driver Number </label>
                            <input type="number" value={driverNumber} required="" onChange={onChangeDriverNumber}   className="form-control" placeholder="Enter Number...." />
                            <div className="clearfix"></div>
                        </div>
                                </ModalBody>
                                <ModalFooter>
                                <Button variant="secondary" onClick={onShowModal2}>
                                        Close
                            </Button>
                                <Button variant="primary" onClick={onSubmitUber}>
                                        Submit
                            </Button>
