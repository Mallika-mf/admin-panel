...
const AddFoodItems = () => {
    ...
    const onClickAddonDelete = (event) =>{
        ...
        let newData = addonList.filter((order,index)=>index !== pushid)
...
<Table>
    ...
    <tbody>
        ...
        <td>{item.price}</td>
        <td><Trash id={index} onClick={onClickAddonDelete} size={15}/></td>
        ...
        {foodTypeList.map((item,index)=>{
            ...
            <tr key = {index}>
                <td>{selectFoodTypeName[selectFoodTypeId.indexOf(item.data)]}</td>
                 <td><Trash id={index} onClick={onDeleteFoodType} size={15}/></td>
            </tr>
            )
        })}
    </tbody>
