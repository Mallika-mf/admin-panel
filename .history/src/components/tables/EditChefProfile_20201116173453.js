...
const AddFoodItems = () => {
 ...
 const [cuisineTypeName,setCuisneTypeName] = useState("")
 ...
 useEffect(()=>{
    app.database().ref().child("Masters").child("Cuisines").once('value').then(function(snapshot){
        if(snapshot.exists){
            ...
            snapshot.forEach(snap =>{
                let val = snap.val()
                cuisinePushId.push(val.PushId)
                cuisinename.push(val.Name)
                let cuisineData={
                    Name: val.Name,
                    PushId: val.PushId
                }
            ...
            selectCuisineId(cuisinePushId)
            setSelectCuisineName(cuisinename)
    ...
    app.database().ref().child("Masters").child("FoodType").once('value').then(function(snapshot){
        if(snapshot.exists){
            ...
            snapshot.forEach(snap =>{
                let val = snap.val()
                foodtypePushId.push(val.PushId)
                foodtypename.push(val.Name)
                let foodtypeData={
                    Name: val.Name,
                    PushId: val.PushId
                }
            ...
            setSelectFoodTypeId(foodtypePushId)
            setSelectFoodTypeName(foodtypename)
...
const onClickSearchHandler = event =>{
     ...
     .once('value').then(function(snapshot){
         ...
         setCloseTime(val.ETime)
         setOpenTime(val.STime)
         setCuisineList(val.Cuisines)
         setFoodTypeList(val.FoodType)
...
const onChangeCuisine = (event) =>{
     ...
     cuisineName.filter(item => {
        if (item.PushId === event.target.value) {
           setCuisneTypeName(item.Name)
           let locker = {
               PushId: event.target.value,
               Name : item.Name
             }
             // addonItem.add(locker)
             setCuisineList([...foodTypeList, locker])
         }
         return item
       })
}
