...
import { Home, Trash, ChevronRight, ArrowDownLeft } from 'react-feather';
...
const CateringList = (props) => {
   ...
   const  onClickSaveMenu=(event)=>{
       ...
       setAddPrice('')
       setAddMenu('')
        app.database().ref().child("CloudKitchen").child(search).child("Packages").child(pushId).child("Items").orderByChild("Menu").equalTo(menuName)
        .on('value',function(snapshot){
            if(snapshot.exists()){
                let content=[]
                snapshot.forEach(snap=>{
                    ...
                    {itemMenu.map((item,index)=>{
                        if(item!==""){
                            return(
                                ...
                                <div className="col-9" ><ChevronRight color="blue" id={item} onClick={onClickAddMenuItem} /><Trash color="red"onClick={() =>onDeleteMenu(index)} /></div>
                                </Row>
                                </Card>
                            )
                        }
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={onAddMenu}>
            Save 
                    ...
                    <>
                    <Card>
                        <CardHeader><ArrowDownLeft onClick={onBackToCustomize}/><h6>Add Item To {menuName}</h6></CardHeader>
                        <CardBody>
          ...
          <Col className="form-group col-md-3">
                                <input type="text" value={addMenu} onChange={onChangeAddMenu} placeholder="Eg:Jeera Rice"/>
                            </Col>
                            <Col className="form-group col-md-3">
                                <input type="number" value={addPrice} onChange={onChangeMenuPrice} onKeyPress={onClickSaveMenu} placeholder="Eg:150"/>
                            </Col>
                        </Row>
