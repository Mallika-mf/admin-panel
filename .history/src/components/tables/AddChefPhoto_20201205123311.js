...
const AddChefPhoto = () => {
    ...
    const onDrop = async(event) => {
        ...
        if(image === '' ) {
          ...
          const uploadTask = await storage.ref("CloudKitchen/").child(name).put(image,metadata)
          ...
          (snapShot) => {
           ...
           const photourl= storage.ref("CloudKitchen/").child(name).getDownloadURL()
           setImageAsUrl(photourl)
            //  .then(fireBaseUrl => {
            //     setImageAsUrl(fireBaseUrl.toString())
