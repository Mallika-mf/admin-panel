...
const AddFoodItems = () => {
  ...
  const onChangeFoodName = (event) => {
    ...
    return firebaseref.once('value').then(function (snapshot) {
      if (snapshot.exists()) {
        ...
        setFoodTypeList(snapshot.val().FoodType)
  ...
  const onUpdateHandler = (event) => {
    ...
    // for (var index1 = 0; index < newFoodList.length; index1++) {
  ...
  return (
    <Fragment>
      ...
      <Container fluid={true}>
        ...
        <Button type="submit" onClick={onDeleteFoodType} className="warning">Delete</Button>
