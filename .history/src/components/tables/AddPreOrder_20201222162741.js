...
const AddPreOrder = () => {
  ...
  const onChangeFoodName = (event) => {
    ...
    return firebaseref.on('value',function (snapshot) {
      if (snapshot.exists()) {
        ...
        if(snapshot.val.Gram!==undefined){
  ...
  const onSubmitHandler = (event) => {
    ...
    if(number!==""){
    let orderDate = new Date(orderDeliveryDate)
      console.log(orderDate)
      let newDate = new Date(orderDate.setDate(orderDate.getDate()+parseInt(number)))
      var dateString =   newDate.getUTCFullYear()+ '-' +(newDate.getMonth()+1)+'-'+newDate.getDate() 
      console.log(String(dateString))
    }else{
      dateString = orderDeliveryDate
    ...
    firebaseref.child("Days").set(number||"0")
    ...
    firebaseref.child("OrderEndDate").set(dateString)
  ...
  const onUpdateHandler = (event) => {
    ...
    if (search === "") {
      ...
      if(number!==""){
        let orderDateUpdate = new Date(orderDeliveryDate)
          console.log(orderDateUpdate)
          let newDateUpdate = new Date(orderDateUpdate.setDate(orderDateUpdate.getDate()+parseInt(number)))
          var dateStringUpdate =   newDateUpdate.getUTCFullYear()+ '-' +(newDateUpdate.getMonth()+1)+'-'+newDateUpdate.getDate() 
          console.log(String(newDateUpdate))
        }else{
          dateStringUpdate = orderDeliveryDate
            console.log(dateStringUpdate)
          }
    ...
    firebaseref.child("RCommision").set("022")
    firebaseref.child("Settlement").set(String(window.ftotal))
    firebaseref.child("OrderEndDate").set(dateStringUpdate)
    firebaseref.child("Status").set("Active")
    firebaseref.child("AStatus").set("InActive")
...
//   }
  return (
    <Fragment>
      <BreadCrumb parent={<Home />} subparent="Food Management" title=" Add PreOrders " />
      <Container fluid={true}>
        ...
        <Row className="form-row">
          <Col className="form-group col-md-3">
            <label className="form-label">Veg/NonVeg/Eggeterian</label>{console.log(veg)}
        ...
        <Row className="form-row">
          <Col className="form-group col-md-3">
            ...
            <Input type="date" id="orderDate" value={orderDeliveryDate} onChange={onChangeOrderDeliveryDate}  min={new Date().toISOString().split('T')[0]} className="form-control" />
            <div className="clearfix"></div>
          </Col>
          </Row>
        ...
        <Row className="form-row">
          <Col className="form-group col-md-3">
            ...
            <Input type="date" value={startDate} onChange={onChangeStartDate} min={new Date().toISOString().split('T')[0]} className="form-control" />
            <div className="clearfix"></div>
          </Col>
          <Col className="form-group col-md-3">
            <label className="form-label">PreOrder End Date <span style={{ color: "red" }}>*</span></label>
            <Input type="date" value={endDate} onChange={onChangeEndtDate} min={new Date().toISOString().split('T')[0]} className="form-control" />
            <div className="clearfix"></div>
          </Col>
          </Row>
        {show === false ?
