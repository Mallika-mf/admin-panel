...
const AddFoodItems = () => {
  ...
  const onSubmitHandler = (event) => {
    ...
    for(let i2=0; i2<newAddonList.rows.length; i2++){
      var objCells = newAddonList.rows.item(i).cells;
     var db = firebaseref.child("ADDONS").push()
      db.child(objCells.item(0).innerHTML).set(objCells.item(1).innerHTML)
