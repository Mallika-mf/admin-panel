...
const AddPreOrder = () => {
  ...
  const onChangeOrderDeliveryDate = (event) => {
    setOrderDeliveryDate(event.target.checked)
  }
  ...
  const onSubmitHandler = (event) => {
    ...
    let orderEndDay = orderDeliveryDate+number
    ...
    // app.database().ref().child("CoudKitchen").child(search).once('value').then(function(snapshot){
      ...
      <Container fluid={true}>
        ...
        <Row className="form-row">
          <Col className="form-group col-md-3">
            ...
            <Input type="date" value={orderDeliveryDate} onChange={onChangeOrderDeliveryDate} className="form-control" />
            <div className="clearfix"></div>
          </Col>
          </Row>
