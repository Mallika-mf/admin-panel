...
const ChefListTable = () => {
  ...
  const onClickRecommendationhHandler=(event)=>{
    setHighLight(event.targte.value)
    searchValue.map(item=>{
      if(item.Highlight===""||item.Highlight====undefined){
        item.Heighlight= "Select"
      }else{
        event.target.value = item.Highlight
      }
    })
                        ...
                        {recordsAfterPagingAndSorting().map((item, id) => {
                          ...
                          return (
                            <TableRow key={id}>
                              ...
                              <TableCell><select  id={item.UserId} value={highlight} onChange={onClickRecommendationhHandler} className="btn btn-success btn-md">
