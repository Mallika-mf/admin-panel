...
const AddPreOrder = () => {
  ...
  const onSubmitHandler = (event) => {
    ...
    // orderEndDay[2] = Number(orderEndDay[2])+parseInt(number)
    // var newDate = orderEndDay.join('-')
    // console.log(newDate)
    //
    if(number!==""){
    ...
    // firebaseref.child("OrderEndDate").set(newDate)
  ...
  const onUpdateHandler = (event) => {
    ...
    if (search === "") {
      ...
      if(number!==""){
        let orderEndDayUpdate = orderDeliveryDate.split('-')
        console.log(orderEndDayUpdate)
        orderEndDayUpdate[2] = Number(orderEndDayUpdate[2])+parseInt(number)
        var newDateUpdate = orderEndDayUpdate.join('-')
        console.log(newDateUpdate)
        }else{
          newDateUpdate = orderDeliveryDate
          console.log(newDateUpdate)
        }
    ...
    firebaseref.child("RCommision").set("022")
    firebaseref.child("Settlement").set(String(window.ftotal))
    firebaseref.child("OrderEndDate").set(newDateUpdate)
    firebaseref.child("Status").set("Active")
    firebaseref.child("AStatus").set("InActive")
