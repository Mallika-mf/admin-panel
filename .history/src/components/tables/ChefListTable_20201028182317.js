...
const ChefListTable = () => {
  ...
  // var passed=false;
    ...
    useEffect(()=>{
        ...
        const [current, setCurrent] = useState(searchValue.slice(count.prev, count.next))

        const getMoreData = () => {
          if (current.length === searchValue.length) {
            setHasMore(false);
            return;
          }
          setTimeout(() => {
            setCurrent(current.concat(searchValue.slice(count.prev + 10, count.next + 10)))
          }, 2000)
