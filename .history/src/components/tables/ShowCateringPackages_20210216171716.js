...
const CateringList = (props) => {
    ...
    const onAddMenu = (event) => {
        setShow(false)
        app.database().ref().child('CloudKitchen').child(search).child("Package").child(pushId).child('ItemMenu').set([...itemMenu, menu].join())
    };
        ...
        <Fragment>
            ...
            <Container fluid={true}>
                    {itemMenu.map((item,index)=>{
                        if(item!==""){
                            return(
                                <Card style={{width:"50px",height:"1000px"}}>
                                <Row>
                                <div className="col-6">
                                <h6>{item}</h6>
                                </div>
                                <div className="col-6" style={{marginLeft:"-20%"}}><button >Add Menu</button></div>
                                </Row>
                                </Card>
                            )
                        }
  ...
  <Modal.Header closeButton>
    <Modal.Title style={{color:"blue"}}>Category</Modal.Title>
  </Modal.Header>

  <Modal.Body>
      <Card>
          <Row>
              <input type="text" value={menu} onChange={onChangeMenu} placeholder="Enter Menu Name"/>
          </Row>
      </Card>
  </Modal.Body>
...
<Modal show={show} onHide={handleClose}>
        ...
        <Modal.Body> <input type="text" value={menu} onChange={onChangeMenu} placeholder="Enter Menu Name"/></Modal.Body>
        <Modal.Footer>
