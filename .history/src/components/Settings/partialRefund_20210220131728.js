            ...
            firebaseref1.transaction(function (currentstock) {
                ...
                function (error, committed, snapshot) {
                    ...
                    if(select === "Dr"){
                        ...
                        .once('value').then(function (snapshot) {
                            axios.post(`https://apps.vibgyortel.in/client/api/sendmessage?apikey=d90d87f44f635906&mobiles=`+snapshot.val().MobileNumber+`&sms=Hello `+snapshot.val().Name+`!%0aDue to the incomplete order(${desc}) we are deducting ${cash} from your wallet %0aYour Chef ID : `+snapshot.val().UserId+` %0a- Team MothersFood&senderid=MOTHFD&schedule=no&sending-time=& client-sms-ids=&unicode=no&message- type=`)
                            .then((res) => {
                              console.log(res);
                              return null;
                          }).catch((err) => {
