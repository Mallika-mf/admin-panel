            ...
            app.database().ref().child("Orders").orderByChild("WorkingPartner").equalTo(username).on("value", function(snapshot) {
               ...
               app.database().ref().child("Orders").orderByChild("WorkingPartner").equalTo(username).on("value", function(snapshot) {
                ...
                snapshot.forEach(function(data){
                    ...
                    if(val.Total!==null)
                    total=total + +parseFloat(val.Total);
                    window.total = total + +parseFloat(val.Total);
                  }); 
                  console.log(total)
                  setUser3("₹"+parseFloat(total).toFixed(2));
                    ...
                    console.log(window.sessionStorage.getItem('com'))
                    let partnerCommision = window.total* +parseFloat(window.sessionStorage.getItem('com'))/100
