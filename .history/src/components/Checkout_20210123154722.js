...
class Checkout extends React.Component {
	...
	handlePlaceOrder = async () => {
		...
		if (parseFloat(subTotal) <= 0) {
			...
			<p>Please Select Your Address First</p>
			</div>)
		}
					...
					<Row>
						...
						<Col md={4}>
							<div className="generator-bg rounded shadow-sm mb-4 p-4 osahan-cart-item">
								...
								<div className="mb-2 bg-white rounded p-2 clearfix">
									<p className="mb-1">Sub Total <span className="float-right text-dark">₹ {subTotal ? subTotal.toFixed(2) : 0.00}</span></p>
                    		...
                    		<OverlayTrigger
									...
									<p className="mb-1">Packing Charges <span className="float-right text-dark">₹ {packingCharges ? parseFloat(packingCharges).toFixed(2) : 0.00}</span></p>

									<p className="mb-1 text-success">Total Discount <span className="float-right text-success"> ₹ {discount1 ? parseFloat(discount1).toFixed(2) : 0.00}</span></p>
									<p className="mb-1 text-success">Taxes &amp; Charges  <span className="float-right text-success">₹ {taxes ? parseFloat(taxes).toFixed(2) : 0.00}</span></p>

									<hr />
									<h6 className="font-weight-bold mb-0">Total Bill:<br />Taxes &amp; Charges Included  <span className="float-right">₹ {total ? total.toFixed(2) : 0.00}</span></h6>
								</div>
