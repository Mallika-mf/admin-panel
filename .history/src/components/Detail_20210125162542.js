...
import { Row, Col, Container, Form,  Button, Tab, Nav, Image, Badge ,Modal} from 'react-bootstrap';
...
class Detail extends React.Component {
			...
			firebase
				...
				.once("value", snapshot => {
					if (snapshot.exists()) {
						...
						// console.log(...this.state.userCart)
						const foodItems = Object.values(snapshot.val().FoodItems ? snapshot.val().FoodItems : {});
						...
						const foodItemsWithQuantity = foodItems.map(obj => {
							if (userCart !== undefined) {
								var cartItemMatch = userCart.find(userCartItem => userCartItem.PushId === obj.PushId)
							}
	...
	if(localFood==="Yes"){
	  return (
		...
		<div className="row">
		<div className="col-3 text-left"><img src={String(image)} style={{width:"50px"}} alt="img"/></div>
	...
	render() {
        ...
        >
		 ...
		 <Modal show={showClearCartModal} onHide={this.handleCancel} 
		 size="lg"
							...
							<Row className="d-flex align-items-end">
								<Col md={8}>
									<div className="restaurant-detailed-header-left" >
										{prd.PP === "https://firebasestorage.googleapis.com/v0/b/mothersfood-a1de8.appspot.com/o/CloudKitchen%2F1599912088639-add.png?alt=media&token=60561b4b-af5d-489c-bbc8-28e8de47851f" || prd.PP === "" ? <Image fluid className="mr-3 float-left" alt="osahan" src="/assets/img/chef.png" style={{marginTop:"-2%"}} /> : <Image fluid className="mr-3 float-left" alt="osahan" style={{ width: '170px', height: '170px', borderRadius: '50%',marginTop:"-5%" }} src={prd.PP} />}
										<h2 className="text-white"style={{ textAlign: "justify" }}>{prd.Name}</h2>
										<h6 className="text-white mb-1"style={{ textAlign: "justify" }}><Icofont icon="icofont-home" /> {prd.KitchenName}<Badge variant="success">OPEN</Badge>
										</h6>

										<p className="text-white mb-0"style={{ textAlign: "justify" }}><Icofont icon="food-cart" /> {cuisinesitems}
										</p>
									</div>
								</Col>
								<Col md={4}>
									<div className="restaurant-detailed-header-right text-right">
										<Button variant='success' type="button"><Icofont icon="clock-time" /> {prd.DeliveryTime !== undefined ? prd.DeliveryTime + " Mins" : "40 Mins"}
										</Button>
										<h6 className="text-white mb-0 restaurant-detailed-ratings">
											...
											</span> Cost for two ₹{prd.CostTwo ? prd.CostTwo : 250}
							...
							<Row>
		                        	  ...
		                        	  <ItemsCarousel /> */}
			                              ...
			                              <Col md={12}>
			                                 <div className="bg-white rounded border shadow-sm mb-4">
												...
												<Row>
													<h5 className="mb-4 mt-3 col-md-12">Food Items <small className="h6 text-black-50">{foodItems.length} &nbsp;Active ITEMS</small></h5>
													<Col md={12}>
														<div className="bg-white rounded border shadow-sm mb-4">
															{foodItems.map((item, index) => {
																if (item.AStatus !== 'Active' || item.Status !== 'Active' || Date.parse(currenttime) < Date.parse(item.SDate) || Date.parse(currenttime) > Date.parse(item.EDate)) {
																...
																return (
																	<>
																		<QuickBite
																			...
																			priceUnit={`₹ `}
																			onIncClicked={() => this.handleDecreaseQuantity(item)}
																			onDecClick={() => this.handleIncreaseQuantity(item)}
																			onAddClick={() => this.handleAddToCart(item)}
								...
								<Col md={4}>
									...
									<div className="generator-bg rounded shadow-sm mb-4 p-4 osahan-cart-item">
										...
										<p className="mb-4 text-white">{userCart.length} &nbsp; Items</p>
						  ...
						  return(
							  <>
											...
											key={index}
												...
												onIncrement={() => this.handleIncreasecartItemQuantity(cartItem)}
										...
										<div className="mb-2 bg-white rounded p-2 clearfix">
											...
											<h6 className="font-weight-bold text-right mb-2">Subtotal :{this.state.num[1]} <span className="text-danger">{userCart.Price}</span></h6>
