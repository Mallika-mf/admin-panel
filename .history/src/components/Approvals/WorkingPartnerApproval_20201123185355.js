  ...
  //  var cno=[];
    ...
    useEffect(()=>{
      ...
      const onApprovalHandler=(event)=>{
          ...
          firebaseref=app.database().ref().child("WebUser").child(pushid);      
          firebaseref.child("AStatus").set("Active");
     ...
     const printDocument=(event)=> {  
        ...
        html2canvas(input)  
          .then((canvas) => {  
            ...
            pdf.save("WorkingPartnerApprovals.pdf");  
          });  
      }
    ...
    return (
        <Fragment>
            <BreadCrumb parent={<Home/>} subparent="Approvals" title="Working Partner Approvals"/>
            <Container fluid={true}>
            {buttonHide===true?
                 <Row>
                 <Col sm="12">
                       <CardHeader>
                             <h6>Working Parner Approvals</h6>
                ...
                table="datatable"  
                filename="WorkingPartnerApprovals"  
                sheet="WorkingPartnerApprovals"  
                buttonText="Excel" />
                <iframe
                    id="iDatatable"
                    src="/approvals/workingPartner-approvals"
                    style={{ display: 'none' }}
                    title="Receipt"
                />
                                    ...
                                    {recordsAfterPagingAndSorting().map((item,id)=>{
                                                // for (var i=0;i<driverNumber.length;i++){
                                                    ...
                                                    return(
                                                       ...
                                                       <td><button id={item.UserName} onClick={onSearchHandler} className="btn btn-primary">{"View Details"}</button></td>
                                                       <td className="actions" style={{textAlign:"center", fontSize: "25px", fontWeight: "bold"}}><button id={item.UserName} onClick={onApprovalHandler}type="button"  className="btn btn-success btn-md">{"Approve"}</button></td>
                                                       <td className="" style={{textAlign:"center", fontSize: "25px", fontWeight: "bold"}}><button id={item.UserName} onClick={onDeleteHandler} type="button"  className="btn btn-danger btn-md">{"Delete"}</button></td>
...
export default WorkingPartnerApproval;