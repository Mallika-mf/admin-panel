...
const ChefProfilePhotoApproval = () => {
        ...
        app.database().ref().child("PhotoRequest").child(userid)
            .once("value", function (snapshot) {
                if (snapshot.exists()) {
                    ...
                    app.database().ref().child("CloudKitchen").child(userid).on('value',snapshot=>{
                        if(snapshot.exists()){
                            if(snapshot.val().Local===undefined||snapshot.val().Local==="No"){
                                app.database().ref().child(snapshot.val().City).child(userid).child("PPApproval").set("Yes");
                                app.database().ref().child(snapshot.val().City).child(userid).child("PP").set(pp)
                            }else{
                                app.database().ref().child("LocalFood").child(snapshot.val().City).child(userid).child("PPApproval").set("Yes");
                                app.database().ref().child("LocalFood").child(snapshot.val().City).child(userid).child("PP").set(pp)
    ...
    return (
        <Fragment>
            <BreadCrumb parent={<Home />} subparent="Approvals" title="Chef Profile Photo Approval" />
            <Container fluid={true}>
                <Row>
                    <Col sm="12">
                        <Card>
                            <CardHeader>
                                <h6>Chef Profile Photo Approval</h6>
