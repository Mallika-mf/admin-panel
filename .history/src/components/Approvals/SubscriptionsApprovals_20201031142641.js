...
const SubscriptionsApprovals = () => {
    ...
    const onSearchHandler=()=>{
        ...
        var database = app.database();
        database.ref().child("Subscriptions")
        .child(search)
        .orderByChild("AStatus").equalTo("InActive")
        .on('value', function(snapshot){
            if(snapshot.exists()){
     ...
     const printDocument=(event)=> {  
        ...
        html2canvas(input)  
          .then((canvas) => {  
            ...
            pdf.save("SusbcriptionsApprovals.pdf");  
          });  
      }
    return (
        <Fragment>
            ...
            <Container fluid={true}>
                <Row>
                    ...
                    <input type="text" value={search} onChange={onChangeSearchHandler} id="city" className="form-control"/>
                    </div>
                ...
                table="datatable"  
                filename="SubscriptionApproval"  
                sheet="SubscriptionApproval"  
                buttonText="Excel" />
                <iframe
                    id="iDatatable"
                    src="/approvals/Subscription-approvals"
                    style={{ display: 'none' }}
                    title="Receipt"
                />
                                            ...
                                            orders.Name.includes(searchTerm)).map((item,id)=>{
                                                // for (var i=0;i<driverNumber.length;i++){
                                                    return(
                                                       ...
                                                       <td className="actions" style={{textAlign:"center"}}><button type="button" className="btn btn-success btn-sm"><Check id={item.PushId+"-"+item.Name+"-"+item.Details+"-"+((+item.Price - (((chefCommision[statecid.indexOf(item.UserId)])*item.Price)/100) - (parseFloat((((chefCommision[statecid.indexOf(item.UserId)])*item.Price)/100)*0.18).toFixed(2))*100)/100)} onClick={onUpdateHandler} size={15}/></button><button type="button" className="btn btn-danger btn-sm"><Trash id={item.UserId} onClick={onDeleteHandler} size={15}/></button></td>
...
export default SubscriptionsApprovals;