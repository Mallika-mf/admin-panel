...
import { Container, Row, Col, Card, CardHeader, Button, Input,Table } from "reactstrap";
...
const ChefApprovals = () => {
  ...
  const [comdays, setComdays] = useState("")
  ...
  const [homeChef, setHomeChef] = useState(false)
  ...
  useEffect(() => {
    ...
    app.database().ref().child("CloudKitchen").orderByChild("AStatus").equalTo("InActive")
      ...
      database.ref().child("CloudKitchen")
        .on('value', function (snapshot) {
          ...
          if (snapshot.exists()) {
            var content = [];
            snapshot.forEach(function(snap){
              // if(snap.val().AStatus==="Active" && snap.val().Franchise.exists()){
              //   child("ApprovedChef").set("yes")
              //}
              // let val = snap.val()
              // let locker = {
              //   UserId: val.UserId,
  ...
  const onClickSearch = (event) =>{
    ...
    window.uid = cId[temp]
    var database = app.database();
    database.ref().child("CloudKitchen").orderByChild("UserId").equalTo(search)
      .on('value', function (snapshot) {
        setUsers([])
  ...
  const onChangeCommisionDate = (event) => {
    setComdays(event.target.value)
  }
  ...
  const homeChefHandler = (event) => {
    setHomeChef(event.target.checked)
  }
  ...
  const onClickSearchHandler = (event) => {
    ...
    return firebaseref.once('value').then(function (snapshot) {
      if (snapshot.exists()) {
        ...
        if(snapshot.val().CommisionDate!==undefined){
          setComdays(snapshot.val().CommisionDate);
  ...
  const onUpdateHandler = (event) => {
    ...
    if(homeChef===false && cateringService===false && localFood===false){
      alert("please selectany of them (HomeChef, Local,Catering)")
      return;
    }
    ...
    firebaseref.child("CommisionDate").set(comdays);
    ...
    if (veg === true)
      ...
      if (homeChef === true){
        firebaseref.child("HomeChef").set("Yes");
      }
    else{
      firebaseref.child("HomeChef").set("No");
      app.database().ref().child(citySelect).child(sname).remove()
    }
      ...
      if (cateringService === true){
        ...
        app.database().ref().child("CateringServices").child(citySelect).child(sname).remove()
      }
    ...
    if (localFood === true){
      ...
      app.database().ref().child("LocalFood").child(citySelect).child(sname).remove()
      ...
      Swal.fire({
        ...
        .then((willDelete) => {
          if (willDelete.value) {
            ...
            firebaseref.child("ApprovalDate").set(today)
            firebaseref.child("ApprovalDateTime").set(time)
            ...
            database.ref().child("CloudKitchen").child(mfid)
              .once('value', function (snapshot) {
                ...
                if(franchise!==""){
                if(val.ApprovedChef===undefined && val.ApprovedChef!=="Yes"){
                  app.database().ref().child("Franchise").child(franchise).once('value',function(franchShot){
                    let snap = franchShot.val()
                    console.log(franchise)
                    console.log(snap)
                    app.database().ref().child("Franchise").child(franchise).child("Chefs").set(snap.Chefs+1)
                    app.database().ref().child("CloudKitchen").child(mfid).child("ApprovedChef").set("Yes")
                  })
                }
              }
              if(val.Local=== "Yes " && val.Catering==="Yes"){
                let firebaseref = app.database().ref().child("LocalFood").child(citypushid).child(mfid);
                let firebaserefCatering = app.database().ref().child("CateringServices").child(citypushid).child(mfid);
                ...
                if (val.Orders !== undefined && val.Orders !== "")
                  ...
                  if (val.SubLocalityName !== undefined && val.SubLocalityName !== "")
                  firebaseref.child("SubLocalityName").set(val.SubLocalityName);
                    else
                    firebaseref.child("SubLocalityName").set("");

                 

                  if(val.Brand!== undefined&& val.Brand!=="")
                  firebaserefCatering.child("Brand").set(val.Brand);
                    else
                    firebaserefCatering.child("Brand").set("");
                  ...
                  if (val.City !== undefined && val.City !== "")
                  firebaserefCatering.child("City").set(val.City);
                    else
                    firebaserefCatering.child("City").set("");

              
                  if (val.Close !== undefined && val.Close !== "")
                  firebaserefCatering.child("Close").set(val.Close);
                    else
                    firebaserefCatering.child("Close").set("");

                 
                  if (val.Commision !== undefined && val.Commision !== "")
                  firebaserefCatering.child("Commision").set(val.Commision);
                    else
                    firebaserefCatering.child("Commision").set("");

               
                  if (val.EDate !== undefined && val.EDate !== "")
                  firebaserefCatering.child("EDate").set(val.EDate);
                    else
                    firebaserefCatering.child("EDate").set("");

                  
                  if (val.Name !== undefined && val.Name !== "")
                  firebaserefCatering.child("Name").set(val.Name);
                    else
                    firebaserefCatering.child("Name").set("");

                  if (val.Location !== undefined && val.Location !== "")
                  firebaserefCatering.child("Location").set(val.Location);
                    else
                    firebaserefCatering.child("Location").set("");

              
                  if (val.Open !== undefined && val.Open !== "")
                  firebaserefCatering.child("Open").set(val.Open);
                    else
                    firebaserefCatering.child("Open").set("");


                  if (val.Status !== undefined && val.Status !== "")
                  firebaserefCatering.child("Status").set(val.Status);
                    else
                    firebaserefCatering.child("Status").set("");

                
                  if (val.AStatus !== null && val.AStatus !== "")
                  firebaserefCatering.child("AStatus").set(val.AStatus);
                    else
                    firebaserefCatering.child("AStatus").set("");

                  
                  if (val.UserId !== undefined && val.UserId !== "")
                  firebaserefCatering.child("UserId").set(val.UserId);
                    else
                    firebaserefCatering.child("UserId").set("");

                  
                  if (val.PP !== undefined && val.PP !== "")
                  firebaserefCatering.child("PP").set(val.PP);
                    else
                    firebaserefCatering.child("PP").set("");
                  ...
                  if (val.Veg !== undefined && val.Veg !== "")
                  firebaserefCatering.child("Veg").set(val.Veg);
                    else
                    firebaserefCatering.child("Veg").set("");

                  
                  if (val.CostTwo !== undefined && val.CostTwo !== "")
                  firebaserefCatering.child("CostTwo").set(val.CostTwo);
                    else
                    firebaserefCatering.child("CostTwo").set("");

                
                  if (val.DeliveryTime !== undefined && val.DeliveryTime !== "")
                  firebaserefCatering.child("DeliveryTime").set(val.DeliveryTime);
                    else
                    firebaserefCatering.child("DeliveryTime").set("");

                 
                  if (val.KitchenName !== undefined && val.KitchenName !== "")
                  firebaserefCatering.child("KitchenName").set(val.KitchenName);
                    else
                    firebaserefCatering.child("KitchenName").set("");

                  
                    if (val.Ratings !== undefined && val.Ratings !== ""){
                      if(val.Ratings!==0){
                        firebaserefCatering.child("Ratings").set(val.Ratings);
      
                      }else{
                        firebaserefCatering.child("Ratings").set(5);
    
                      }
                    }else{
                      firebaserefCatering.child("Ratings").set("");
    
                    }

                 
                  if (val.FoodType !== undefined && val.FoodType !== "")
                  firebaserefCatering.child("FoodType").set(val.FoodType);
              

                  if (val.Cuisines !== undefined && val.Cuisines !== "")
                  firebaserefCatering.child("Cuisines").set(val.Cuisines);
                 

                  if (val.Orders !== undefined && val.Orders !== "")
                  firebaserefCatering.child("Orders").set(val.Orders);
                    else
                    firebaserefCatering.child("Orders").set("");

                    if (val.SubLocalityName !== undefined && val.SubLocalityName !== "")
                  firebaserefCatering.child("SubLocalityName").set(val.SubLocalityName);
                    else
                    firebaserefCatering.child("SubLocalityName").set("");

              }else if(val.Local==="No" && val.Catering=== "Yes"){
                let firebaseref = app.database().ref().child(citypushid).child(mfid);
                let firebaserefCatering = app.database().ref().child("CateringServices").child(citypushid).child(mfid);
               ...
               console.log(val.Catering)
                }
                else if(val.Catering==="Yes"){
                  var firebaseref = app.database().ref().child("CateringServices").child(citypushid).child(mfid);
                  console.log("okay go it")
        ...
        {hide !== true ?
          <div>
            ...
            <Col sm="12">
              ...
              <Row >
                ...
                <Col className="form-group col-md-4">
                <label className="form-label">Select Date for Commision Free Days<span style={{ color: "red" }}>*</span> </label>
                <Input type="date" id="commision" value={comdays} onChange={onChangeCommisionDate} className="form-control" />
                <div className="clearfix"></div>
              </Col>
              </Row>
              ...
              <Col className="form-group col-md-4">
                <Input type="checkbox" checked={homeChef} onChange={homeChefHandler} className="form-control" />HomeChef
                         <div className="clearfix"></div>
              </Col>
              <Row>
                <Col className="form-group col-md-12">
          ...
          : <div>
            ...
            <div className="form-group row">
                    ...
                    <span id="search" onClick={onClickSearch}><img src="https://img.icons8.com/ios-filled/24/000000/search.png" alt="search engine"/></span>
                    </div>
                    </div>
                     <div className="clearfix"></div>
                    </div>
            ...
            <Col sm="12">
              <div className="col-md-12" style={{ margin: "1%" }}>
                ...
                {/* <div className="form-group col-md-8">
                  ...
                  <Input type="text" placeholder="Search for Chef Id..." value={search} onChange={handleSearch} required="" className="form-control" />
                  <span id="search" style={{position:"relative",marginLeft:"105%",marginBottom :'50%'}} onClick={onClickSearch}><img src="https://img.icons8.com/ios-filled/24/000000/search.png" alt="search engine"/></span>
              </div> */}
              </div>
