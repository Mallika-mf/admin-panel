    ...
    const printIframe = (id) => {
     ...
     const printDocument=(event)=> {  
        ...
        const onSubmitLocationCoordinates = (event) =>{
            ...
            app.database().ref().child('CloudKitchen').child(arr[0]).child('Location').set(latlng)
            app.database().ref().child('CloudKitchen').child(arr[0]).once('value',function(snapshot){
                let val = snapshot.val()
                if(val.Local==="Yes"){
                    app.database().ref().child('LocalFood').child(val.City).child(arr[0]).child('Location').set(latlng)

                }else{
                    app.database().ref().child(val.City).child(arr[0]).child('Location').set(latlng)
        ...
        <Fragment>
            ...
            <Row>
                      ...
                      <div className="col-md-4">
                        ...
                        <input className="btn btn-primary mr-1" style={{ marginTop: "30px", padding: "10px 15px" }} type="button" name="locationcord" value="locationcord" onClick={(event) => onSubmitLocationCoordinates(event)} id="locationcord" />
