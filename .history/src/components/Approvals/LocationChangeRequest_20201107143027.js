...
const LocationChangeApprovals = () => {
    ...
    const onSaveHandler=(event)=>{
        ...
        var database = app.database();
        database.ref().child("CloudKitchen").child(pushid)
        .on('value', function(snapshot){
            if(snapshot.exists()){
                var content = [];
                ...
                if(snapshot.val().Local!==undefined&& snapshot.val().Local==="Yes"){
                    app.database().ref().child("LocalFood").child(snapshot.val().City).child(pushid).child("Location").set(location);
                 }else{
                    app.database().ref().child(snapshot.val().City).child(pushid).child("Location").set(location);
        ...
        console.log(cityId)
        app.database().ref().child("CloudKitchen").child(pushid).child("Location").set(location);
                                       ...
                                       {users.filter(orders=>orders.UserId.includes(search)).map((item,id)=>{
                                           return(
                                               <tr key={id}>
                                                   ...
                                                   <td>{item.Address}</td>
                                                   <td>{item.Coord}</td>
                                                   <td style={{fontSize: "25px", fontWeight: "bold"}}><button type="button" id={item.UserId+","+item.Coord+","+item.UserId} onClick={onSaveHandler} className="btn btn-primary btn-md">Save</button></td>
                                                   <td style={{fontSize: "25px", fontWeight: "bold"}}><button type="button" id={item.UserId} onClick={onDeleteHandler} className="btn btn-primary btn-md">Delete</button></td>
                                               </tr>
                                           )
                                       })}
                                    </tbody>
