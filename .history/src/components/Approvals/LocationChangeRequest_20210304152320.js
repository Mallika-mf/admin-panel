            ...
            .on('value', function(snapshot){
                ...
                .once('value').then(function (snapshot) {
                    ...
                    snapshot.forEach(function (data) {
                        var val = data.val();
                        if (val.UserId !== "" && val.UserId !== undefined) {
                            cid.push(val.UserId);
    ...
    const printIframe = (id) => {
     ...
     const printDocument=(event)=> {  
        ...
        const onSubmitLocationCoordinates = (event) =>{
            ...
            //         console.log(cID[i])
            if(latlng === ""){
                alert("please enter LocationCordinates")
                return;
            }
            if(chefId === ""){
            ...
            Swal.fire({
                title: "Are you sure want to change the location?",
                text: "",
                icon: "warning",
            ...
            app.database().ref().child('CloudKitchen').child(chefId).once('value',function(snapshot){
                if(snapshot.exists()){
                    ...
                    if(snapshot.val().Local!==undefined&& snapshot.val().Local==="Yes"){
                        app.database().ref().child("LocalFood").child(snapshot.val().City).child(snapshot.val().UserId).child("Location").set(latlng);
                     }else{
                        app.database().ref().child(snapshot.val().City).child(snapshot.val().UserId).child("Location").set(latlng);
            ...
            Swal.fire( {
                icon: "success",
                title:"location changed for "+chefId,
              });
        }
    })
    //     }else{
        ...
        <Fragment>
            ...
            <FormGroup className="col-md-3">
                      ...
                      <div className="col-md-4">
                        ...
                        <input className="btn btn-primary mr-1" style={{ marginTop: "30px", padding: "10px 15px" }} type="button" name="Save" value="Save" onClick={(event) => onSubmitLocationCoordinates(event)} id="Save" />
