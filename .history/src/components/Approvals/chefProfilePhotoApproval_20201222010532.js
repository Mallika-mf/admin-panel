        ...
        app.database().ref().child("PhotoRequest").child(userid)
            .once("value", function (snapshot) {
                if (snapshot.exists()) {
                    ...
                    if(local===undefined||local==="No"){
                        app.database().ref().child(pushid).child(userid).child("PP").set(pp)
                    }else{
                        app.database().ref().child("LocalFood").child(pushid).child(userid).child("PP").set(pp)
                                        ...
                                        {recordsAfterPagingAndSorting().map((item, id) => {
                                            return (
                                                <tr key={id}>
                                                    ...
                                                    <td className="actions" style={{ fontSize: "25px", fontWeight: "bold" }}><button type="button" id={item.UserId+","+item.City+","+item.Local} onClick={onClickApprovalHandler} className="btn btn-primary btn-md">Approve</button></td>
