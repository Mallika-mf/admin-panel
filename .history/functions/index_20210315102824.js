...
exports.ChotabetaRequest = functions.database.ref(`/Orders/{pushid}/Chotabeta`)
  .onCreate((change, context) => {
    ...
    return admin.database().ref("/Orders/" + pushid + "/").once('value').then(snap => {
      ...
      var resturantname = "" 
      {
        snap.child("OrderType").val()==="Local" ?
      resturantname=snap.child("KitchenName").val():
      resturantname=snap.child("ChefName").val();
    }
      ...
      return axios({
        ...
        data: {
          ...
          "fav_store": `hub_name:${pickaddress},location:${picklat + ";" + picklng},store_incharge:${resturantname},contact:${resturantphone}`,
          "item_name": itemName,
        ...
        // if(response.data.message === "Success"){
          ...
          if (response.data.order_status_string === "Driver Ongoing") {
            ref.child("ChotabetaOrderId").set(clientorderid);
    
            ref.child("DeliveryApiRemarks").set("Chotabeta: Started for picking the Order");
            ref.child("DeliveryName").set(response.data.rider_name);
            ref.child("DeliveryNumber").set(response.data.rider_number);
            console.log(response.data.order_status_string)
          }
          else if (response.data.order_status_string === "DE is on the way to Delivery") {
            ref.child("DeliveryApiRemarks").set("Chotabeta :  Driver is on the way");
            ref.child("DeliveryName").set(response.data.rider_name);
            ref.child("DeliveryNumber").set(response.data.rider_number);
            console.log(response.data.order_status_string)
          ...
          else if (response.data.order_status_string === "Driver Pickedup") {
            ref.child("DeliveryApiRemarks").set("Chotabeta: Pickedup the order");
            ref.child("Status").set("4");
            console.log(response.data.order_status_string)

          }
          else if (response.data.order_status_string === "Driver Started") {
            ref.child("DeliveryApiRemarks").set("Chotabeta : Started for delivery");
            console.log(response.data.order_status_string)

          }
          else if (response.data.order_status_string === "Arrived") {
            ref.child("DeliveryApiRemarks").set("Chotabeta : Reached to  delivery location");
            console.log(response.data.order_status_string)

          }
          else if (response.data.order_status_string === "Delivered") {
            ref.child("Status").set("5");
            ref.child("DeliveryApiRemarks").set("Chotabeta :  Delivered");
            console.log(response.data.order_status_string)

          }
          else if (response.data.order_status_string === "Order Cancelled") {
...
exports.GetChotabetaBStatusUpdate = functions.https.onRequest((request, response) => {

  if (request.method === 'POST') {
    console.log(request);
    var clientorderid = request.body.order_id;
    var status = request.body.order_status_string;
    var statusCode = request.body.order_status_code
    ...
    return query.once('value', (snapshot) => {
      ...
      else if (status === "DE is on the way to Delivery") {
        ref.child("DeliveryApiRemarks").set("Chotabeta : Driver is on the way ");
        ref.child("DeliveryName").set(request.body.rider_name);
        ref.child("DeliveryNumber").set(request.body.rider_number);
        console.log(status)
      ...
      return axios({
        ...
        if (response.data.success === "false") {
          ...
          if(response.data.message==="Order Failed!"){
            ...
            admin.database().ref("/Orders/" + pushid).child("PickkupOrderId").set(response.data.data.orderId);
          }
  ...
  if (request.method === 'POST') {
    ...
    return query.once('value', (snapshot) => {
      ...
      if (status === 1001) {
        ...
        ref.child("DeliveryApiRemarks").set(`Pickkup : ${request.body.orderStatus}`);
        ref.child("DeliveryName").set(request.body.agentDetails.agentName);
        ref.child("DeliveryNumber").set(request.body.agentDetails.agentMobile);
        ref.child("PickUpETA").set(request.body.eta);


      }
      else if (status === 1003) {
        ref.child("DeliveryApiRemarks").set(`Pickkup : ${request.body.orderStatus}`);
        ref.child("DeliveryLat").set(request.body.agentDetails.agentLatitude);
        ref.child("DeliveryLng").set(request.body.agentDetails.agentLongitude);
        ref.child("DeliveryAddress").set(request.body.agentDetails.address);
      }
      else if (status === 1004) {
        ref.child("Status").set("4");
        ref.child("DeliveryApiRemarks").set(`Pickkup : ${request.body.orderStatus}`);
        ...
        ref.child("Status").set("5");
        ref.child("DeliveryApiRemarks").set(`Pickkup : ${request.body.orderStatus}`);
        ref.child("DropPhoto").set(request.body.dropPhoto);
...
.onUpdate((change,context)=>{
    ...
    return admin.database().ref("/Orders/"+pushid+"/").once('value').then(snap => {
        ...
        if(status==='5'){
          var today1 = new Date();
         let inidantime =  today1.toLocaleString('en-US', {timeZone: "Asia/Kolkata"});
          console.log(inidantime);
          return admin.database().ref('Orders').child(pushid).child("DeliveryDateTime").set(""+inidantime);
        }
        else return;
