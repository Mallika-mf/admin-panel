  ...
  if (request.method === 'POST') {
    ...
    return query.once('value', (snapshot) => {
      ...
      if (status === 1001) {
        ...
        ref.child("DeliveryLat").set(request.body.agentDetails.agentMobile);
        ref.child("DeliveryLng").set(request.body.agentDetails.agentMobile);
        ref.child("DeliveryAddress").set(request.body.agentDetails.agentMobile);
      }
      else if (status === 1004) {
        ref.child("Status").set("4");
        ref.child("DeliveryApiRemarks").set(`Pickkup : ${request.body.orderStatus}`);
