    ...
    return admin.database().ref("/Orders/" + pushid + "/").once('value').then(snap => {
      ...
      return axios({
        ...
        if (response.data.info["status"] === "Error") {
          ...
          admin.database().ref("/Orders/" + pushid).child("DeliveryApiRemarks").set("Chotabeta : " + response.data.info["message"]);
          ...
          admin.database().ref("/Orders/" + pushid).child("ChotabetaOrderId").set(response.data.data["order_id"]);
...
exports.PickkupRequest = functions.database.ref(`/Orders/{pushid}/Pickkup`)
  .onCreate((change, context) => {
    ...
    return admin.database().ref("/Orders/" + pushid + "/").once('value').then(snap => {
      ...
      return axios({
        method: 'post',
        url: 'https://tstops.pickkup.com/api/create_order?apiKey=nq1fZTkOoKM@CUL2XJV@adSyCxU$cI3nTb2McTonAO',
        ...
        data: {
          ...
          "dropLocation": {
            ...
            // Drop Location complete Address
            "dropLat": droplat, // Latitude of the Drop Address
            "dropLng":droplng, // Longitude of the Drop Address
          ...
          if (response.data.info["success"] === "false") {
            console.log(response.data.info["message"]);
            admin.database().ref("/Orders/" + pushid).child("DeliveryApiRemarks").set("Pickkup : " + response.data.info["message"]);
            // admin.database().ref("/PendingOrders/"+pushid).set(snap.val());
          }
          else {
            console.log("Riders Available");
            ...
            admin.database().ref("/Orders/"+pushid).child("PickUpPhoto").set(response.data.data["pickPhoto"]);
            admin.database().ref("/Orders/"+pushid).child("DropPhoto").set(response.data.data["dropPhoto"]);
            admin.database().ref("/Orders/"+pushid).child("DeliveryLat").set(response.data.data["agentLatitude"]);
            admin.database().ref("/Orders/"+pushid).child("DeliveryLng").set(response.data.data["agentLongitude"]);
            admin.database().ref("/Orders/"+pushid).child("DeliveryAddress").set(response.data.data["address"]);
